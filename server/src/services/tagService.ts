...
export async function listTagsAsync(userId: number) {
  return asyncDb.prepare('SELECT * FROM tags WHERE user_id = ? ORDER BY name ASC').all(userId);
}
...
export async function createTagAsync(userId: number, name: string, color?: string) {
  const result = await asyncDb
    .prepare('INSERT INTO tags (user_id, name, color) VALUES (?, ?, ?)')
    .run(userId, name, color || '#10b981');
  return asyncDb.prepare('SELECT * FROM tags WHERE id = ?').get(result.lastInsertRowid);
}
...
export async function getTagByIdAndUserAsync(tagId: number | string, userId: number) {
  return asyncDb.prepare('SELECT * FROM tags WHERE id = ? AND user_id = ?').get(tagId, userId);
}
...
export async function updateTagAsync(tagId: number | string, name?: string, color?: string) {
  await asyncDb
    .prepare('UPDATE tags SET name = COALESCE(?, name), color = COALESCE(?, color) WHERE id = ?')
    .run(name || null, color || null, tagId);
  return asyncDb.prepare('SELECT * FROM tags WHERE id = ?').get(tagId);
}
...
export async function deleteTagAsync(tagId: number | string) {
  await asyncDb.prepare('DELETE FROM tags WHERE id = ?').run(tagId);
}
