import { Download, ExternalLink, MapPin, Pencil, RotateCcw, Star, StickyNote, Ticket, Trash2 } from 'lucide-react';
    ...
    >
            ...
            return (
              <div
                style={{
                  ...
                  width: '100%',
                  height: '100%',
                  background: isPdf ? '#ef44441a' : 'var(--bg-tertiary)',
                }}
              >
                <span
                  style={{
                    fontSize: 9,
                    fontWeight: 700,
                    color: isPdf ? '#ef4444' : 'var(--text-muted)',
                    letterSpacing: 0.3,
                  }}
                >
                  {ext}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          {!isTrash && file.starred ? (
            <Star size={12} fill="#facc15" color="#facc15" style={{ flexShrink: 0 }} />
          ) : null}
          <span
            onClick={() => !isTrash && openFile(file)}
            style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
          {file.file_size && (
            <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>{formatSize(file.file_size)}</span>
          )}
          ...
          {linkedReservations.map((r) =>
            TRANSPORT_TYPES.has(r.type) ? (
              <SourceBadge
                key={r.id}
                icon={transportIcon(r.type)}
                label={`${t('files.sourceTransport')} · ${r.title || t('files.sourceTransport')}`}
              />
            ) : (
              <SourceBadge
                key={r.id}
                icon={Ticket}
                label={`${t('files.sourceBooking')} · ${r.title || t('files.sourceBooking')}`}
              />
            )
          )}
          {file.note_id && <SourceBadge icon={StickyNote} label={t('files.sourceCollab') || 'Collab Notes'} />}
        </div>
      </div>
        ...
        {isTrash ? (
          ...
          <>
            <button
              ...
              title={file.starred ? t('files.unstar') || 'Unstar' : t('files.star') || 'Star'}
              style={{
                padding: 6,
                background: 'none',
                border: 'none',
                cursor: 'pointer',
                color: file.starred ? '#facc15' : 'var(--text-faint)',
                borderRadius: 6,
                display: 'flex',
              }}
              onMouseEnter={(e) => {
                if (!file.starred) e.currentTarget.style.color = '#facc15';
              }}
              onMouseLeave={(e) => {
                if (!file.starred) e.currentTarget.style.color = 'var(--text-faint)';
              }}
            >
            ...
            {can('file_edit', trip) && (
              <button
                onClick={() => setAssignFileId(file.id)}
                title={t('files.assign') || 'Assign'}
                style={{
                  padding: 6,
                  background: 'none',
                  border: 'none',
