...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i trippi.ai.',
    manage: 'Hantera egenskaper under Inställningar',
    madeWith: 'Gjorde med',
    openTrippi: 'Öppna trippi.ai',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna trippi.ai för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny trippi.ai version tillgänglig',
      body: `trippi.ai ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt trippi.ai konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
