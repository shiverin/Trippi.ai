...
export function useDayDetail(
  ...
  useEffect(() => {
    ...
    setLoading(true);
    weatherApi
      .getDetailed(lat, lng, day.date, language)
  ...
  useEffect(() => {
    if (day) setHotelDayRange({ start: day.id, end: day.id });
  }, [day?.id]);
  ...
  const handleSaveAccommodation = async () => {
    ...
    try {
      ...
      setDayAccommodations(
        updated.filter((a) => (day ? isDayInAccommodationRange(day, a.start_day_id, a.end_day_id, days) : false))
      );
