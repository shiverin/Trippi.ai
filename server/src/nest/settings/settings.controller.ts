...
export class SettingsController {
  ...
  @Put()
  async upsert(@CurrentUser() user: User, @Body() body: { key?: string; value?: unknown }) {
    if (!body.key) {
  ...
  async bulk(@CurrentUser() user: User, @Body() body: { settings?: unknown }) {
    if (!body.settings || typeof body.settings !== 'object') {
      throw new HttpException({ error: 'Settings object is required' }, 400);
    }
    try {
      const updated = await this.settings.bulkUpsertSettings(user.id, body.settings as Record<string, unknown>);
