...
describe('VacayPersons', () => {
  ...
  it('FE-COMP-VACAYPERSONS-009: Selecting a preset color calls updateColor', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Change color' })).toBeInTheDocument());
    ...
    const colorSwatches = allBtns.filter((b) => b.style.backgroundColor && !b.textContent?.trim() && !b.title);
