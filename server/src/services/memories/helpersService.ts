...
export function canAccessTrippiPhoto(requestingUserId: number, trekPhotoId: number): boolean {
  const photo = db.prepare('SELECT * FROM trippi_photos WHERE id = ?').get(trekPhotoId) as
