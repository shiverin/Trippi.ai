...
export default function DevNotificationsPanel(): React.ReactElement {
  ...
  const Btn = ({
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-xs text-content-faint">{sub}</p>
      </div>
      {sending === id && (
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <h3 className="mb-3 text-sm font-semibold text-content-secondary">{children}</h3>
  );
  ...
  const TripSelector = () => (
    <select
      ...
      className="mb-3 w-full rounded-lg border border-edge bg-surface-card px-3 py-2 text-sm text-content"
    >
      {trips.map((trip) => (
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-2">
        <div className="rounded bg-[#fbbf24] px-2 py-0.5 font-mono text-xs font-bold text-[#000]">DEV ONLY</div>
        <span className="text-sm font-medium text-content">Notification Testing</span>
      </div>
      ...
      {users.length > 0 && (
        <div>
          ...
          <p className="mb-3 text-xs text-content-muted">Fires each user event to the selected recipient.</p>
          <UserSelector />
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <Btn
      ...
      <div>
        ...
        <p className="mb-3 text-xs text-content-muted">Fires to all admin users.</p>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Btn
