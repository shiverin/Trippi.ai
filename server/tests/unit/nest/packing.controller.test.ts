...
describe('PackingController (parity with the legacy /api/trips/:tripId/packing route)', () => {
  it('404 when the trip is not accessible', async () => {
    ...
    expect(await thrown(() => new PackingController(svc).list(user, '5'))).toEqual({
  ...
  describe('POST / (create)', () => {
    it('403 without packing_edit permission', async () => {
      const svc = makeService({ canEdit: vi.fn().mockReturnValue(false) });
      expect(await thrown(() => new PackingController(svc).create(user, '5', { name: 'Socks' }))).toEqual({
    ...
    it('400 when name missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).create(user, '5', {}))).toEqual({
        status: 400, body: { error: 'Item name is required' },
      });
    });

    it('creates an item and broadcasts', async () => {
      ...
      expect(await new PackingController(svc).create(user, '5', { name: 'Socks' }, 'sock')).toEqual({ item: { id: 9, name: 'Socks' } });
      expect(broadcast).toHaveBeenCalledWith('5', 'packing:created', { item: { id: 9, name: 'Socks' } }, 'sock');
    });
  });

  it('GET / lists items for the trip (success path)', async () => {
  ...
  describe('POST /import', () => {
    it('400 when items is not a non-empty array (empty array)', async () => {
      const svc = makeService();
      expect(await thrown(() => new PackingController(svc).importItems(user, '5', []))).toEqual({
        status: 400, body: { error: 'items must be a non-empty array' },
      });
    });

    it('400 when items is not an array at all (non-array branch)', async () => {
      const svc = makeService();
      expect(await thrown(() => new PackingController(svc).importItems(user, '5', 'nope'))).toEqual({
        status: 400, body: { error: 'items must be a non-empty array' },
      });
    });
    ...
    it('403 without packing_edit permission', async () => {
      const svc = makeService({ canEdit: vi.fn().mockReturnValue(false) });
      expect(await thrown(() => new PackingController(svc).importItems(user, '5', [{ name: 'a' }]))).toEqual({
        status: 403, body: { error: 'No permission' },
      });
    });

    it('imports and broadcasts per item', async () => {
      const bulkImport = vi.fn().mockReturnValue([{ id: 1 }, { id: 2 }]);
      const broadcast = vi.fn();
      const svc = makeService({ bulkImport, broadcast } as Partial<PackingService>);
      const res = await new PackingController(svc).importItems(user, '5', [{ name: 'a' }, { name: 'b' }], 'sock');
  ...
  describe('PUT /:id (update)', () => {
    it('404 when the item is missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).update(user, '5', '9', { name: 'X' }))).toEqual({
        status: 404, body: { error: 'Item not found' },
      });
    });

    it('updates, forwards changed keys, and broadcasts', async () => {
  ...
  describe('PUT /reorder', () => {
    it('reorders the items and reports success', async () => {
      ...
      expect(await new PackingController(svc).reorder(user, '5', [3, 1, 2])).toEqual({ success: true });
      expect(reorderItems).toHaveBeenCalledWith('5', [3, 1, 2]);
    });

    it('403 without packing_edit permission', async () => {
      const svc = makeService({ canEdit: vi.fn().mockReturnValue(false) });
      expect(await thrown(() => new PackingController(svc).reorder(user, '5', [1]))).toEqual({
        status: 403, body: { error: 'No permission' },
      });
    });
  });
  ...
  describe('DELETE /:id (remove)', () => {
    it('404 when the item is missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).remove(user, '5', '9'))).toEqual({
        status: 404, body: { error: 'Item not found' },
      });
    });
  ...
  describe('bags', () => {
    it('GET /bags lists bags for the trip', async () => {
      const listBags = vi.fn().mockReturnValue([{ id: 3, name: 'Carry-on' }]);
      const svc = makeService({ listBags } as Partial<PackingService>);
      expect(await new PackingController(svc).listBags(user, '5')).toEqual({ bags: [{ id: 3, name: 'Carry-on' }] });
    });

    it('400 on bag create with blank name', async () => {
      const svc = makeService();
      expect(await thrown(() => new PackingController(svc).createBag(user, '5', { name: '  ' }))).toEqual({
        status: 400, body: { error: 'Name is required' },
      });
    });

    it('400 on bag create with no name at all (optional-chain short-circuit)', async () => {
      const svc = makeService();
      expect(await thrown(() => new PackingController(svc).createBag(user, '5', {}))).toEqual({
        status: 400, body: { error: 'Name is required' },
      });
    });
    ...
    it('404 on bag update when missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).updateBag(user, '5', '3', { name: 'X' }))).toEqual({
    ...
    it('404 on bag delete when missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).deleteBag(user, '5', '3'))).toEqual({
        status: 404, body: { error: 'Bag not found' },
      });
    });
    ...
    it('404 on set-members when the bag is missing', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).setBagMembers(user, '5', '3', [1, 2]))).toEqual({
        status: 404, body: { error: 'Bag not found' },
      });
    });

    it('sets bag members and broadcasts (array branch)', async () => {
      ...
      const res = await new PackingController(svc).setBagMembers(user, '5', '3', [1, 2], 'sock');
    ...
    it('coerces non-array members to an empty list (ternary else branch)', async () => {
      const setBagMembers = vi.fn().mockReturnValue([]);
      const svc = makeService({ setBagMembers } as Partial<PackingService>);
      await new PackingController(svc).setBagMembers(user, '5', '3', 'not-an-array');
  ...
  describe('templates', () => {
    ...
    it('404 when applying a missing/empty template (POST stays 200 otherwise)', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).applyTemplate(user, '5', 't1'))).toEqual({
    ...
    it('400 when an admin saves a template with no name (whitespace)', async () => {
      const saveAsTemplate = vi.fn();
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      expect(await thrown(() => new PackingController(svc).saveAsTemplate(admin, '5', '   '))).toEqual({
        status: 400, body: { error: 'Template name is required' },
      });
      expect(saveAsTemplate).not.toHaveBeenCalled();
    });

    it('400 when an admin saves a template with no name at all (optional-chain)', async () => {
      const saveAsTemplate = vi.fn();
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      expect(await thrown(() => new PackingController(svc).saveAsTemplate(admin, '5'))).toEqual({
        status: 400, body: { error: 'Template name is required' },
    ...
    it('403 when a non-admin tries to save a template', async () => {
      const saveAsTemplate = vi.fn();
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      expect(await thrown(() => new PackingController(svc).saveAsTemplate(user, '5', 'My template'))).toEqual({
    ...
    it('400 when an admin saves a template with no items', async () => {
      ...
      expect(await thrown(() => new PackingController(svc).saveAsTemplate(admin, '5', 'My template'))).toEqual({
    ...
    it('saves a template for an admin', async () => {
      ...
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      expect(await new PackingController(svc).saveAsTemplate(admin, '5', 'My template')).toEqual({
  ...
  describe('category assignees', () => {
    ...
    it('decodes the URI-encoded category name before forwarding', async () => {
      ...
      await new PackingController(svc).updateCategoryAssignees(user, '5', 'Toys%20%26%20Games', [2]);
    ...
    it('updates assignees, broadcasts and fires the tag notification', async () => {
      ...
      const notifyTagged = vi.fn();
      const svc = makeService({ updateCategoryAssignees, broadcast, notifyTagged } as Partial<PackingService>);
      const res = await new PackingController(svc).updateCategoryAssignees(user, '5', 'Clothes', [2], 'sock');
