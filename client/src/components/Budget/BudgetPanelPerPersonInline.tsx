...
import { colorForUserId, fmtNum, widgetTheme } from './BudgetPanel.helpers';
...
export default function PerPersonInline({
  tripId,
  budgetItems,
  currency,
  locale,
  grandTotal,
  theme,
}: PerPersonInlineProps & { grandTotal: number; theme: ReturnType<typeof widgetTheme> }) {
          ...
          return (
            <div key={p.user_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '6px 0' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13.5, fontWeight: 500, letterSpacing: '-0.01em', color: theme.text }}>
                  {p.username}
                </div>
