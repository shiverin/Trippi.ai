...
Set `DEMO_MODE=true` in your environment and restart trippi.ai. See [Environment-Variables](Environment-Variables) for how to set environment variables.
...
trippi.ai schedules an automatic hourly reset of the demo database. At each reset:
...
On first start with demo mode active, trippi.ai seeds three example trips (Tokyo & Kyoto, Barcelona Long Weekend, New York City) owned by the admin and shared with the demo user, then saves the initial baseline automatically.
