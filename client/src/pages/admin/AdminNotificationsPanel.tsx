...
import { ADMIN_CHANNEL_LABEL_KEYS, ADMIN_EVENT_LABEL_KEYS } from './AdminPage.constants';
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <div className="p-6">
          {saving && (
            <p className="text-content-faint" style={{ fontSize: 11, marginBottom: 8 }}>
              Saving…
            </p>
          )}
          {/* Header row */}
              ...
              >
                ...
                {visibleChannels.map((ch) => {
                  if (!implementedForEvent.includes(ch)) {
                    return (
                      <span key={ch} className="text-content-faint" style={{ textAlign: 'center', fontSize: 14 }}>
                        —
                      </span>
                    );
                  }
                  ...
                  return (
                    <div key={ch} style={{ display: 'flex', justifyContent: 'center' }}>
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 flex-shrink-0 items-center rounded-full transition-colors ${isOn ? 'bg-content' : 'bg-edge'}`}
                      >
                        <span
