...
describe('getMergedItems', () => {
  ...
  it('keeps a post-dinner midnight item after earlier evening items', () => {
    const dayAssignments = [
      { id: 1, order_index: 0, place: { place_time: '08:00' } },
      { id: 2, order_index: 1, place: { place_time: '18:00' } },
      { id: 3, order_index: 2, place: { place_time: '00:00' } },
    ];

    const result = getMergedItems({ dayAssignments, dayNotes: [], dayTransports: [], dayId: 5 });

    expect(result.map((i) => i.data.id)).toEqual([1, 2, 3]);
  });
});
