import type {
  VacayAddYearRequest,
  VacayCompanyHolidayRequest,
  VacayInviteActionRequest,
  VacayInviteRequest,
  VacaySetColorRequest,
...
import type { HolidaysMap, VacayEntry, VacayHolidayCalendar, VacayPlan, VacayStat, VacayUser } from '../types';
...
interface VacayApi {
  ...
  addHolidayCalendar: (data: {
    region: string;
    color?: string;
    label?: string | null;
  }) => Promise<{ calendar: VacayHolidayCalendar }>;
  updateHolidayCalendar: (
    id: number,
    data: { region?: string; color?: string; label?: string | null }
  ) => Promise<{ calendar: VacayHolidayCalendar }>;
...
const api: VacayApi = {
  ...
  updateColor: (color, targetUserId) =>
    ax
      .put('/addons/vacay/color', { color, target_user_id: targetUserId } satisfies VacaySetColorRequest)
      .then((r: AxiosResponse) => r.data),
  invite: (userId) =>
    ax
      .post('/addons/vacay/invite', { user_id: userId } satisfies VacayInviteRequest)
      .then((r: AxiosResponse) => r.data),
  acceptInvite: (planId) =>
    ax
      .post('/addons/vacay/invite/accept', { plan_id: planId } satisfies VacayInviteActionRequest)
      .then((r: AxiosResponse) => r.data),
  declineInvite: (planId) =>
    ax
      .post('/addons/vacay/invite/decline', { plan_id: planId } satisfies VacayInviteActionRequest)
      .then((r: AxiosResponse) => r.data),
  cancelInvite: (userId) =>
    ax.post('/addons/vacay/invite/cancel', { user_id: userId }).then((r: AxiosResponse) => r.data),
  ...
  addYear: (year) =>
    ax.post('/addons/vacay/years', { year } satisfies VacayAddYearRequest).then((r: AxiosResponse) => r.data),
  ...
  toggleEntry: (date, targetUserId) =>
    ax
      .post('/addons/vacay/entries/toggle', { date, target_user_id: targetUserId } satisfies VacayToggleEntryRequest)
      .then((r: AxiosResponse) => r.data),
  toggleCompanyHoliday: (date) =>
    ax
      .post('/addons/vacay/entries/company-holiday', { date } satisfies VacayCompanyHolidayRequest)
      .then((r: AxiosResponse) => r.data),
  getStats: (year) => ax.get(`/addons/vacay/stats/${year}`).then((r: AxiosResponse) => r.data),
  updateStats: (year, days, targetUserId) =>
    ax
      .put(`/addons/vacay/stats/${year}`, {
        vacation_days: days,
        target_user_id: targetUserId,
      } satisfies VacayUpdateStatsRequest)
      .then((r: AxiosResponse) => r.data),
  ...
  addHolidayCalendar: (data) =>
    ax.post('/addons/vacay/plan/holiday-calendars', data).then((r: AxiosResponse) => r.data),
  updateHolidayCalendar: (id, data) =>
    ax.put(`/addons/vacay/plan/holiday-calendars/${id}`, data).then((r: AxiosResponse) => r.data),
  deleteHolidayCalendar: (id) =>
    ax.delete(`/addons/vacay/plan/holiday-calendars/${id}`).then((r: AxiosResponse) => r.data),
};
...
export const useVacayStore = create<VacayState>((set, get) => ({
  ...
  updatePlan: async (updates: Partial<VacayPlan>) => {
    ...
    set({ plan: data.plan });
    await Promise.all([get().loadEntries(), get().loadStats(), get().loadHolidays()]);
  },

  updateColor: async (color: string, targetUserId?: number) => {
    await api.updateColor(color, targetUserId);
    await Promise.all([get().loadPlan(), get().loadEntries()]);
  },
  ...
  loadHolidays: async (year?: number) => {
    ...
    const calendarsByCountry = new Map<string, VacayHolidayCalendar[]>();
    for (const cal of calendars) {
      const country = cal.region.split('-')[0];
      const countryCalendars = calendarsByCountry.get(country) ?? [];
      countryCalendars.push(cal);
      calendarsByCountry.set(country, countryCalendars);
    }

    const holidayResults = await Promise.all(
      Array.from(calendarsByCountry.entries()).map(async ([country, countryCalendars]) => {
        try {
          return { countryCalendars, data: await api.getHolidays(y, country) };
        } catch {
          return null;
        }
      })
