...
function ensureDir(): void {
  if (!fs.existsSync(TRIPPI_PHOTO_DIR)) {
    fs.mkdirSync(TRIPPI_PHOTO_DIR, { recursive: true });
  }
}

function cachedFilePath(key: string): string {
  return path.join(TRIPPI_PHOTO_DIR, `${key}.bin`);
}
...
export function getFresh(key: string): { filePath: string; contentType: string } | null {
  ...
  if (Date.now() - row.fetched_at >= CACHE_TTL) {
    db.prepare('DELETE FROM trippi_photo_cache_meta WHERE cache_key = ?').run(key);
    return null;
  }
