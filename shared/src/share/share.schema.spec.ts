...
describe('shareLinkRequestSchema', () => {
  it('accepts any subset of section toggles (all optional) and an empty body', () => {
    ...
    expect(shareLinkRequestSchema.safeParse({ profile_visible: true }).success).toBe(true);
  ...
  it('rejects a non-boolean toggle', () => {
    ...
    expect(shareLinkRequestSchema.safeParse({ profile_visible: 'yes' }).success).toBe(false);
  });
});
