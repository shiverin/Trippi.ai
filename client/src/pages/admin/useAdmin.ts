...
import type { AdminStats, AdminUser, OidcConfig, UpdateInfo } from './adminModel';
...
export function useAdmin() {
  ...
  const [editForm, setEditForm] = useState<{ username: string; email: string; plan: string; password: string }>({
    username: '',
    email: '',
    plan: 'free',
    password: '',
  ...
  const [createForm, setCreateForm] = useState<{ username: string; email: string; password: string; plan: string }>({
    username: '',
    email: '',
    password: '',
    plan: 'free',
  ...
  const handleSaveUser = async () => {
    try {
      const payload: { username?: string; email?: string; plan: string; password?: string } = {
