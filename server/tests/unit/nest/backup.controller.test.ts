...
describe('BackupController', () => {
  it('GET /list returns backups, 500 on error', () => {
    expect(
      new BackupController(
        svc({ listBackups: vi.fn().mockReturnValue([{ filename: 'a.zip' }]) } as Partial<BackupService>),
      ).list(),
    ).toEqual({ backups: [{ filename: 'a.zip' }] });
    expect(
      thrown(() =>
        new BackupController(
          svc({
    ...
    ).toEqual({ status: 500, body: { error: 'Error loading backups' } });
  });

  it('POST /create 429 when rate-limited, else creates + audits', async () => {
    expect(
      await thrownAsync(() =>
        new BackupController(svc({ checkRateLimit: vi.fn().mockReturnValue(false) })).create(user, req),
      ),
    ).toEqual({ status: 429, body: { error: 'Too many backup requests. Please try again later.' } });
  ...
  it('GET /download 400 invalid / 404 missing, else res.download', () => {
    ...
    expect(
      thrown(() =>
        new BackupController(svc({ isValidBackupFilename: vi.fn().mockReturnValue(false) })).download('x', res),
      ),
    ).toEqual({ status: 400, body: { error: 'Invalid filename' } });
    expect(
      thrown(() =>
        new BackupController(svc({ backupFileExists: vi.fn().mockReturnValue(false) })).download('x.zip', res),
      ),
    ).toEqual({ status: 404, body: { error: 'Backup not found' } });
  ...
  it('POST /restore maps the service status, else audits', async () => {
    expect(
      await thrownAsync(() =>
        new BackupController(svc({ isValidBackupFilename: vi.fn().mockReturnValue(false) })).restore(user, 'x', req),
      ),
    ).toEqual({ status: 400, body: { error: 'Invalid filename' } });
    expect(
      await thrownAsync(() =>
        new BackupController(svc({ backupFileExists: vi.fn().mockReturnValue(false) })).restore(user, 'x.zip', req),
      ),
    ).toEqual({ status: 404, body: { error: 'Backup not found' } });
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({
            restoreFromZip: vi.fn().mockResolvedValue({ success: false, status: 422, error: 'bad zip' }),
          } as Partial<BackupService>),
        ).restore(user, 'x.zip', req),
      ),
    ).toEqual({ status: 422, body: { error: 'bad zip' } });
  ...
  it('POST /restore falls back to status 400 when the service omits one', async () => {
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({
            restoreFromZip: vi.fn().mockResolvedValue({ success: false, error: 'nope' }),
          } as Partial<BackupService>),
        ).restore(user, 'x.zip', req),
      ),
  ...
  it('POST /upload-restore 400 without a file, cleans up the tmp file', async () => {
    expect(await thrownAsync(() => new BackupController(svc()).uploadRestore(user, undefined, req))).toEqual({
      status: 400,
  ...
  it('POST /upload-restore maps a failed restore status', async () => {
    ...
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({
            restoreFromZip: vi.fn().mockResolvedValue({ success: false, status: 422, error: 'bad' }),
          } as Partial<BackupService>),
        ).uploadRestore(user, file, req),
      ),
    ).toEqual({ status: 422, body: { error: 'bad' } });
  });
  ...
  it('POST /upload-restore falls back to a default name and maps unexpected errors to 500', async () => {
    ...
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({ restoreFromZip: vi.fn().mockRejectedValue(new Error('boom')) } as Partial<BackupService>),
        ).uploadRestore(user, file, req),
      ),
    ...
    expect(writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'backup.upload_restore', resource: 'upload.zip' }),
    );
  });
  ...
  it('maps unexpected service errors to 500 (create, restore, auto-settings)', async () => {
    ...
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({ createBackup: vi.fn().mockRejectedValue(new Error('disk')) } as Partial<BackupService>),
        ).create(user, req),
      ),
    ).toEqual({ status: 500, body: { error: 'Error creating backup' } });
    expect(
      await thrownAsync(() =>
        new BackupController(
          svc({ restoreFromZip: vi.fn().mockRejectedValue(new Error('boom')) } as Partial<BackupService>),
    ...
    ).toEqual({ status: 500, body: { error: 'Could not load backup settings' } });
  });
  ...
  it('PUT /auto-settings tolerates a missing body', () => {
    ...
    new BackupController(svc({ updateAutoSettings } as Partial<BackupService>)).updateAutoSettings(
      user,
  ...
  it('GET/PUT /auto-settings', () => {
    expect(
      new BackupController(
        svc({
          getAutoSettings: vi.fn().mockReturnValue({ settings: { enabled: true }, timezone: 'UTC' }),
        } as Partial<BackupService>),
      ).autoSettings(),
    ).toEqual({ settings: { enabled: true }, timezone: 'UTC' });
    const res = new BackupController(
      svc({
        updateAutoSettings: vi.fn().mockReturnValue({ enabled: true, interval: 'daily', keep_days: 7 }),
      } as Partial<BackupService>),
    ).updateAutoSettings(user, { enabled: true }, req);
  ...
  it('DELETE /:filename 400/404, else deletes + audits', () => {
    expect(
      thrown(() =>
        new BackupController(svc({ isValidBackupFilename: vi.fn().mockReturnValue(false) })).remove(user, 'x', req),
      ),
    ).toEqual({ status: 400, body: { error: 'Invalid filename' } });
    expect(
      thrown(() =>
        new BackupController(svc({ backupFileExists: vi.fn().mockReturnValue(false) })).remove(user, 'x.zip', req),
      ),
    ).toEqual({ status: 404, body: { error: 'Backup not found' } });
    const deleteBackup = vi.fn();
    expect(new BackupController(svc({ deleteBackup } as Partial<BackupService>)).remove(user, 'x.zip', req)).toEqual({
      success: true,
    });
