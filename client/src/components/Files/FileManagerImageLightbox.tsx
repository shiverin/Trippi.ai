...
export function ImageLightbox({ files, initialIndex, onClose }: ImageLightboxProps) {
  ...
  const navBtn = (side: 'left' | 'right', onClick: () => void, show: boolean): React.ReactNode =>
    show ? (
      <button
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        ...
        {imgSrc && (
          <img
            ...
            style={{ maxWidth: '85vw', maxHeight: '80vh', objectFit: 'contain', borderRadius: 8, display: 'block' }}
            onClick={(e) => e.stopPropagation()}
          />
        )}
...
function ThumbImg({
  ...
  useEffect(() => {
    getAuthUrl(file.url, 'download').then(setSrc);
  }, [file.url]);
  return (
    <button
      onClick={onClick}
