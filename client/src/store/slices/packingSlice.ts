...
export const createPackingSlice = (set: SetState, get: GetState): PackingSlice => ({
  ...
  togglePackingItem: async (tripId, id, checked) => {
    set((state) => ({
      packingItems: state.packingItems.map((item) => (item.id === id ? { ...item, checked: checked ? 1 : 0 } : item)),
    }));
    try {
      ...
      set((state) => ({
        packingItems: state.packingItems.map((item) => (item.id === id ? { ...item, checked: checked ? 0 : 1 } : item)),
      }));
