  ...
  const handleTestProvider = async (provider: PhotoProviderAddon) => {
    ...
    try {
      ...
      if (ok) {
        ...
        toast.error(
          `${t('memories.connectionError', { provider_name: provider.name })} ${res.data?.error ? `: ${String(res.data.error)}` : ''}`
        );
      }
    } catch {
              ...
              {field.input_type === 'checkbox' ? (
                <div className="flex items-center gap-3">
                  <ToggleSwitch
                    ...
                    onToggle={() =>
                      handleProviderFieldChange(provider.id, field.key, values[field.key] === 'true' ? 'false' : 'true')
                    }
                  />
                ...
                <>
                  ...
                  <input
                    ...
                    placeholder={
                      field.secret && connected && !(values[field.key] || '') ? '••••••••' : field.placeholder || ''
                    }
                  ...
                  />
                  {field.hint && <p className="mt-1 text-xs text-slate-500">{t(`memories.${field.hint}`)}</p>}
                </>
              )}
            </div>
          ))}
            ...
            <button
              ...
              title={
                !canSave
                  ...
                  : isProviderSaveDisabled(provider)
                    ? t('memories.fillRequiredFields')
                    : ''
              }
            >
