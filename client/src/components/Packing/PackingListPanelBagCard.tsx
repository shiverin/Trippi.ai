...
interface BagCardProps {
  ...
  onSetMembers: (bagId: number, userIds: number[]) => void;
  t: any;
  compact?: boolean;
}
...
export function BagCard({
  ...
  const sz = compact
    ? { dot: 10, name: 12, weight: 11, bar: 6, count: 10, gap: 6, mb: 14, icon: 11, avatar: 18 }
    : { dot: 12, name: 14, weight: 13, bar: 8, count: 11, gap: 8, mb: 16, icon: 13, avatar: 22 };
  ...
  return (
    <div style={{ marginBottom: sz.mb }}>
      ...
      >
        {(bag.members || []).map((m) => (
          <span
            ...
            style={{ cursor: canEdit ? 'pointer' : 'default', display: 'inline-flex' }}
          >
            {m.avatar ? (
              <img
                src={m.avatar}
              ...
              <span
                style={{
                  ...
                  height: sz.avatar,
                  borderRadius: '50%',
                  background: bag.color + '25',
                  color: bag.color,
                  fontSize: sz.avatar * 0.45,
                  fontWeight: 700,
                  display: 'inline-flex',
                  alignItems: 'center',
      ...
      <div style={{ fontSize: sz.count, color: 'var(--text-faint)', marginTop: 2 }}>
