...
const hu: NotificationLocale = {
  email: {
    footer: 'Ezt az értesítést azért kaptad, mert engedélyezted az értesítéseket a trippi.ai-ben.',
    manage: 'Beállítások kezelése',
    madeWith: 'Made with',
    openTrippi: 'trippi.ai megnyitása',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a trippi.ai-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Új trippi.ai verzió érhető el',
      body: `A trippi.ai ${p.version} elérhető. Látogasson el az adminisztrációs panelre a frissítéshez.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Szia',
    body: 'Kérést kaptunk a trippi.ai-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.',
