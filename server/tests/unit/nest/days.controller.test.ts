...
describe('DaysController (parity with the legacy /api/trips/:tripId/days route)', () => {
  it('404 when trip not accessible', async () => {
    ...
    expect(await thrown(() => new DaysController(svc).list(user, '5'))).toEqual({ status: 404, body: { error: 'Trip not found' } });
  });
  ...
  it('POST / 403 without day_edit, then creates + broadcasts', async () => {
    expect(await thrown(() => new DaysController(daysSvc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', {}))).toEqual({ status: 403, body: { error: 'No permission' } });
  ...
  it('POST / 404 when the trip is not accessible', async () => {
    const svc = daysSvc({ verifyTripAccess: vi.fn().mockReturnValue(null) });
    expect(await thrown(() => new DaysController(svc).create(user, '5', {}))).toEqual({ status: 404, body: { error: 'Trip not found' } });
  });
  ...
  describe('PUT /reorder', () => {
    it('404 when the trip is not accessible', async () => {
      const svc = daysSvc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) });
      expect(await thrown(() => new DaysController(svc).reorder(user, '5', { orderedIds: [1, 2] }))).toEqual({ status: 404, body: { error: 'Trip not found' } });
    });

    it('403 without day_edit', async () => {
      const svc = daysSvc({ canEdit: vi.fn().mockReturnValue(false) });
      expect(await thrown(() => new DaysController(svc).reorder(user, '5', { orderedIds: [1, 2] }))).toEqual({ status: 403, body: { error: 'No permission' } });
    });

    it('400 when orderedIds is missing', async () => {
      expect(await thrown(() => new DaysController(daysSvc()).reorder(user, '5', {}))).toEqual({ status: 400, body: { error: 'orderedIds must be an array' } });
    });

    it('400 when orderedIds is not an array', async () => {
      expect(await thrown(() => new DaysController(daysSvc()).reorder(user, '5', { orderedIds: 'nope' as never }))).toEqual({ status: 400, body: { error: 'orderedIds must be an array' } });
    });
  ...
  it('PUT /:id 404 when the day is missing, else updates', async () => {
    expect(await thrown(() => new DaysController(daysSvc({ getDay: vi.fn().mockReturnValue(undefined) } as Partial<DaysService>)).update(user, '5', '9', {}))).toEqual({ status: 404, body: { error: 'Day not found' } });
  ...
  it('DELETE /:id 404 when missing, else success', async () => {
    expect(await thrown(() => new DaysController(daysSvc({ getDay: vi.fn().mockReturnValue(undefined) } as Partial<DaysService>)).remove(user, '5', '9'))).toEqual({ status: 404, body: { error: 'Day not found' } });
...
describe('DayNotesController (parity with the legacy /api/.../days/:dayId/notes route)', () => {
  it('400 on an over-long text BEFORE the trip-access check (middleware order)', async () => {
    const verifyTripAccess = vi.fn().mockReturnValue(undefined); // would 404 if reached
    const svc = notesSvc({ verifyTripAccess });
    expect(await thrown(() => new DayNotesController(svc).create(user, '5', '3', { text: 'x'.repeat(501) }))).toEqual({
      status: 400, body: { error: 'text must be 500 characters or less' },
    });
    expect(verifyTripAccess).not.toHaveBeenCalled();
  });

  it('400 on an over-long time', async () => {
    expect(await thrown(() => new DayNotesController(notesSvc()).create(user, '5', '3', { text: 'ok', time: 'y'.repeat(251) }))).toEqual({
      status: 400, body: { error: 'time must be 250 characters or less' },
    });
  });

  it('404 trip, 403 permission, 404 day, 400 empty text, then creates', async () => {
    expect(await thrown(() => new DayNotesController(notesSvc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).create(user, '5', '3', { text: 'ok' }))).toEqual({ status: 404, body: { error: 'Trip not found' } });
    expect(await thrown(() => new DayNotesController(notesSvc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', '3', { text: 'ok' }))).toEqual({ status: 403, body: { error: 'No permission' } });
    expect(await thrown(() => new DayNotesController(notesSvc({ dayExists: vi.fn().mockReturnValue(false) } as Partial<DayNotesService>)).create(user, '5', '3', { text: 'ok' }))).toEqual({ status: 404, body: { error: 'Day not found' } });
    expect(await thrown(() => new DayNotesController(notesSvc({ dayExists: vi.fn().mockReturnValue(true) } as Partial<DayNotesService>)).create(user, '5', '3', { text: '  ' }))).toEqual({ status: 400, body: { error: 'Text required' } });
  ...
  it('GET / returns notes; PUT/DELETE 404 when the note is missing', async () => {
    ...
    expect(await thrown(() => new DayNotesController(notesSvc({ getNote: vi.fn().mockReturnValue(undefined) } as Partial<DayNotesService>)).update(user, '5', '3', '9', { text: 'x' }))).toEqual({ status: 404, body: { error: 'Note not found' } });
    expect(await thrown(() => new DayNotesController(notesSvc({ getNote: vi.fn().mockReturnValue(undefined) } as Partial<DayNotesService>)).remove(user, '5', '3', '9'))).toEqual({ status: 404, body: { error: 'Note not found' } });
  });
