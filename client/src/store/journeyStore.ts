...
export interface JourneyPhoto {
  ...
  shared: number;
  created_at: number;
  // Joined from trippi_photos for display
  provider?: string;
...
interface JourneyState {
  ...
  uploadGalleryPhotos: (
    ...
    files: File[],
    cbs?: { onProgress?: (p: UploadProgress) => void }
  ) => Promise<ResilientResult<GalleryPhoto>>;
...
export const useJourneyStore = create<JourneyState>((set, get) => ({
  ...
  updateEntry: async (entryId, data) => {
    ...
    set((s) => {
      ...
      return {
        current: { ...s.current, entries: s.current.entries.map((e) => (e.id === entryId ? { ...e, ...updated } : e)) },
      };
    });
  },
  ...
  reorderEntries: async (journeyId, orderedIds) => {
    ...
    set((s) => {
      ...
      const entries = s.current.entries.map((e) => (sortMap.has(e.id) ? { ...e, sort_order: sortMap.get(e.id)! } : e));
      entries.sort((a, b) => {
