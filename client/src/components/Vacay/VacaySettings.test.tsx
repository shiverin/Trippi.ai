...
beforeEach(() => {
  ...
  server.use(
    ...
    http.get('/api/addons/vacay/holidays/:year/:country', () => HttpResponse.json([]))
  );
});
...
describe('VacaySettings', () => {
  ...
  it('FE-COMP-VACAYSETTINGS-005: clicking an active weekend day removes it', async () => {
    ...
    const dayButtons = screen.getAllByRole('button').filter((b) => b.style.padding === '4px 10px');
  ...
  it('FE-COMP-VACAYSETTINGS-008: countries are loaded from API and shown in selector', async () => {
    ...
    const selectButton = allButtons.find(
      (b) => b.textContent?.includes('vacay.selectCountry') || b.textContent?.includes('country')
    );
    if (selectButton) {
  ...
  it('FE-COMP-VACAYSETTINGS-009: dissolve section shown only when isFused', () => {
    ...
    const buttons = screen.getAllByRole('button');
    const dissolveButton = buttons.find(
      (b) => b.className.includes('bg-red-500') || b.className.includes('bg-red-600')
    );
    ...
    const dissolveButtonAfter = buttonsAfter.find(
      (b) => b.className.includes('bg-red-500') || b.className.includes('bg-red-600')
    );
  ...
  it('FE-COMP-VACAYSETTINGS-011: calendar row shows delete button and calls deleteHolidayCalendar', async () => {
    ...
    const trashButton = buttons.find((b) => b.className.includes('p-1.5') && b.className.includes('shrink-0'));
  ...
  it('FE-COMP-VACAYSETTINGS-012: calendar row color picker opens on color button click', async () => {
    ...
    const colorButton = screen
      .getAllByRole('button')
      .find((b) => b.style.width === '28px' && b.style.height === '28px');
    expect(colorButton).toBeDefined();
    await user.click(colorButton!);

    // Color picker should now be visible (12 preset color swatches with width:24)
    const swatches = screen.getAllByRole('button').filter((b) => b.style.width === '24px' && b.style.height === '24px');
  ...
  it('FE-COMP-VACAYSETTINGS-016: carry-over toggle calls updatePlan', async () => {
    ...
    const toggleButtons = screen
      .getAllByRole('button')
      .filter((b) => b.className.includes('inline-flex') && b.className.includes('rounded-full'));
