...
// ─────────────────────────────────────────────────────────────────────────────
// Trip overview endpoint — GET /api/trips/:id/overview
// ─────────────────────────────────────────────────────────────────────────────

describe('Trip overview', () => {
  it('OVERVIEW-001 — returns 404 when the user cannot access the trip', async () => {
    const { user: owner } = createUser(testDb);
    const { user: other } = createUser(testDb);
    const trip = createTrip(testDb, owner.id, { title: 'Private Trip' });

    const res = await request(app).get(`/api/trips/${trip.id}/overview`).set('Cookie', authCookie(other.id));
  ...
  it('OVERVIEW-002 — returns a schema-compatible empty trip overview', async () => {
    ...
    expect(res.status).toBe(200);
    const parsed = tripOverviewResponseSchema.parse(res.body);
    expect(parsed.overview.trip.title).toBe('Empty Trip');
    expect(parsed.overview.boards.find((board) => board.id === 'decisions')?.count).toBe(0);
  ...
  it('OVERVIEW-003 — builds boards from explicit backend sources without keyword-only inflation', async () => {
    ...
    const days = testDb.prepare('SELECT * FROM days WHERE trip_id = ? ORDER BY day_number').all(trip.id) as any[];
    const flight = createReservation(testDb, trip.id, {
    ...
    testDb
      .prepare(
        "UPDATE reservations SET status = 'pending', confirmation_number = NULL, reservation_time = ?, needs_review = 1 WHERE id = ?",
      )
      .run('2026-12-07T09:00:00', flight.id);
    const hotel = createReservation(testDb, trip.id, {
      title: 'Hotel deposit',
      type: 'hotel',
      day_id: days[0].id,
    });
    testDb
      .prepare(
        "UPDATE reservations SET status = 'confirmed', confirmation_number = 'HTL-1', reservation_time = ? WHERE id = ?",
      )
      .run('2026-12-07T18:00:00', hotel.id);
    testDb
      .prepare(
        "INSERT INTO trip_files (trip_id, reservation_id, filename, original_name, mime_type) VALUES (?, ?, 'hotel.pdf', 'hotel.pdf', 'application/pdf')",
      )
      .run(trip.id, hotel.id);

    testDb
      .prepare(
        "INSERT INTO booking_intents (trip_id, created_by, type, origin, destination, status, watch_status) VALUES (?, ?, 'flight', 'SFO', 'PEK', 'options_ready', 'idle')",
      )
      .run(trip.id, owner.id);

    testDb
      .prepare(
        "INSERT INTO group_decisions (trip_id, created_by, title, deadline, state) VALUES (?, ?, 'Choose hotpot night', '2026-12-01', 'open')",
      )
      .run(trip.id, owner.id);
    testDb
      .prepare(
        "INSERT INTO todo_items (trip_id, name, checked, category, sort_order, due_date) VALUES (?, 'Upload visa PDFs', 0, 'Documents', 0, '2026-12-01')",
      )
      .run(trip.id);
    testDb
      .prepare(
        "INSERT INTO todo_items (trip_id, name, checked, category, sort_order) VALUES (?, 'Book passport decision words only', 0, NULL, 1)",
      )
      .run(trip.id);

    const budget = createBudgetItem(testDb, trip.id, { name: 'Hotel deposit', total_price: 320 });
    testDb
      .prepare('INSERT INTO budget_item_members (budget_item_id, user_id, paid) VALUES (?, ?, 0)')
      .run(budget.id, member.id);
    createPackingItem(testDb, trip.id, { name: 'Passport', category: 'Documents' });

    const res = await request(app).get(`/api/trips/${trip.id}/overview`).set('Cookie', authCookie(member.id));

    expect(res.status).toBe(200);
    const parsed = tripOverviewResponseSchema.parse(res.body);
    const board = (id: string) => parsed.overview.boards.find((item) => item.id === id)!;

    expect(board('decisions').count).toBe(1);
    expect(board('bookings').count).toBe(2);
    expect(board('budget').count).toBeGreaterThanOrEqual(1);
    expect(board('packing').count).toBeGreaterThanOrEqual(1);
    expect(board('files').count).toBe(2);
    expect(board('files').summary).toContain('document follow-up');
    expect(board('decisions').items.map((item) => item.title)).not.toContain('Book passport decision words only');
    expect(board('bookings').items.map((item) => item.title)).not.toContain('Book passport decision words only');
    expect(board('files').items.map((item) => item.title)).toContain('Upload visa PDFs');
    expect(board('files').items.map((item) => item.title)).toContain('Flight to Beijing');
  });
});
