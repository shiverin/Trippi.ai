...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  useEffect(() => {
    ...
    const bookends = day && optimizeFromAccommodation !== false ? getDayBookendHotels(day, days, accommodations) : null;
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Tagesliste */}
      <div
        className={`scroll-container${draggingId ? '' : 'trek-stagger'}`}
      ...
      >
        {days.map((day, index) => {
          ...
          const routeBookends =
            optimizeFromAccommodation !== false ? getDayBookendHotels(day, days, accommodations) : null;
                  ...
                  {merged.length === 0 && !dayNoteUi ? (
                    ...
                    merged.map((item, idx) => {
                      ...
                      if (item.type === 'place') {
                        ...
                        const transportPlaceRouteActive = routeShown && !hiddenAssignmentRouteIds.has(assignment.id);
                            ...
                            >
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {(() => {
                                  ...
                                  const active =
                                    hasRouteToggle && routeShown ? visibleConnectionIds.includes(res.id) : false;
                          ...
                          <React.Fragment key={`transport-${res.id}-${legKey}-${day.id}`}>
                            ...
                            >
                              ...
                              {onToggleConnection &&
                                ...
                                (res.endpoints || []).length >= 2 &&
                                (() => {
                                  const active = routeShown ? visibleConnectionIds.includes(res.id) : false;
                                  const routeLabel = !bookingRoutesGlobalShown
                                    ? t('map.enableAllConnectionsFirst')
                                    : !routeShown
                                      ? 'Show this day route first'
