import { Maximize2, Pencil, Pin, PinOff, Trash2 } from 'lucide-react';
...
export function NoteCard({
  ...
  const author = note.author ||
    note.user || {
      ...
      avatar: note.avatar_url || (note.avatar ? `/uploads/avatars/${note.avatar}` : null),
    };
        ...
        {/* Hover actions in header */}
        {
          ...
          >
            {note.content && (
              <button
                onClick={() => onView?.(note)}
                title={t('collab.notes.expand') || 'Expand'}
                style={{
                  padding: 3,
                  background: 'none',
                  border: 'none',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    {(note.attachments?.length || 0) > (note.website ? 1 : 2) && (
                      <span style={{ fontSize: 8, color: 'var(--text-faint)', textAlign: 'center' }}>
                        +{(note.attachments?.length || 0) - (note.website ? 1 : 2)}
                      </span>
                    )}
                  </div>
