...
describe('TripsController (parity with the legacy /api/trips route)', () => {
  it('GET / lists for the user with the archived flag', async () => {
    ...
    expect(await new TripsController(svc({ list } as Partial<TripsService>)).list(user, '1')).toEqual({
      trips: [{ id: 1 }],
    });
  ...
  describe('POST / (create)', () => {
    it('403 without trip_create, 400 without title', async () => {
      expect(
        await thrown(() =>
      ...
      expect(await thrown(() => new TripsController(svc()).create(user, {}, req))).toEqual({
        status: 400,
  ...
  it('GET /:id returns the trip when present', async () => {
    ...
    expect(await new TripsController(s).get(user, '9')).toEqual({ trip: { id: 9 } });
  });
  ...
  describe('PUT /:id', () => {
    it('404 when no access; 403 on archive without trip_archive', async () => {
      ...
      expect(await thrown(() => new TripsController(s).update(user, '9', { is_archived: 1 }, req))).toEqual({
        status: 403,
    ...
    it('403 on cover_image without trip_cover_upload', async () => {
      ...
      expect(await thrown(() => new TripsController(s).update(user, '9', { cover_image: '/x.jpg' }, req))).toEqual({
        status: 403,
    ...
    it('403 on an edit field without trip_edit', async () => {
      ...
      expect(await thrown(() => new TripsController(s).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 403,
    ...
    it('maps a NotFoundError to 404 and a ValidationError to 400', async () => {
      ...
      expect(await thrown(() => new TripsController(nf).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const ve = svc({
      ...
      expect(await thrown(() => new TripsController(ve).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 400,
        body: { error: 'bad' },
      });
    });
  ...
  describe('POST /:id/copy', () => {
    it('403 without trip_create, 404 without access', async () => {
      expect(
        await thrown(() =>
          new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).copy(user, '9', undefined, req),
        ),
      ).toEqual({ status: 403, body: { error: 'No permission to create trips' } });
      expect(
        await thrown(() =>
  ...
  describe('DELETE /:id', () => {
    it('404 when no owner, 403 without trip_delete', async () => {
      ...
      expect(await thrown(() => new TripsController(s).remove(user, '9', req))).toEqual({
        status: 403,
  ...
  describe('members', () => {
    it('GET 404 without access, else owner+members+current_user_id', async () => {
      ...
      expect(await new TripsController(s).members(user, '9')).toEqual({
        owner: { id: 1 },
        members: [],
        current_user_id: 1,
      });
    ...
    it('POST 403 without member_manage, else adds + notifies', async () => {
      expect(
        await thrown(() =>
          new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).addMember(user, '9', 'bob@x.y'),
        ),
      ...
      expect(await new TripsController(s).addMember(user, '9', 'bob@x.y')).toEqual({
        member: { id: 2, email: 'bob@x.y' },
      });
    ...
    it('POST 404 without trip access', async () => {
      ...
      expect(await thrown(() => new TripsController(s).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 404,
    ...
    it('POST maps NotFoundError to 404, ValidationError to 400, re-throws others', async () => {
      ...
      expect(await thrown(() => new TripsController(nf).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 404,
        body: { error: 'no user' },
      });
      const ve = svc({
      ...
      expect(await thrown(() => new TripsController(ve).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 400,
    ...
    it('DELETE 404 without trip access', async () => {
      const s = svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) });
      expect(await thrown(() => new TripsController(s).removeMember(user, '9', '2'))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
    ...
    it('DELETE self needs no permission; removing others needs member_manage', async () => {
      ...
      expect(await new TripsController(s).removeMember(user, '9', '1')).toEqual({ success: true });
      expect(await thrown(() => new TripsController(s).removeMember(user, '9', '2'))).toEqual({
        status: 403,
  ...
  it('GET /:id/bundle 404 then aggregates', async () => {
    ...
    expect(await new TripsController(s).bundle(user, '9')).toEqual({ trip: { id: 9 }, days: [] });
  });

  describe('POST /:id/cover', () => {
    const file = { filename: 'abc.jpg' } as Express.Multer.File;
    it('404 without access, 403 without permission, 404 raw trip, 400 no file, else returns url', async () => {
      expect(
        await thrown(() =>
      ...
      expect(
        await thrown(() => new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).cover(user, '9', file)),
      ...
      expect(await new TripsController(s).cover(user, '9', file)).toEqual({ cover_image: '/uploads/covers/abc.jpg' });
    ...
    it('403 in demo mode for a demo account', async () => {
      ...
      try {
        expect(await thrown(() => new TripsController(svc()).cover(user, '9', file))).toEqual({
          status: 403,
  ...
  describe('GET /:id/export.ics', () => {
    ...
    it('maps a NotFoundError from the export to 404 and re-throws others', async () => {
      ...
      expect(await thrown(() => new TripsController(nf).exportIcs(user, '9', makeRes()))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const other = svc({
  ...
  it('POST /:id/copy maps a copy failure to 500', async () => {
    ...
    expect(await thrown(() => new TripsController(s).copy(user, '9', undefined, req))).toEqual({
      status: 500,
