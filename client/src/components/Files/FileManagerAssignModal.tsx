import { Check, MapPin, Ticket, X } from 'lucide-react';
        ...
        <div style={{ overflowY: 'auto', padding: 8 }}>
          {(() => {
            ...
            for (const day of days) {
              ...
              const dayPlaces = da
                .map((a) => places.find((p) => p.id === a.place?.id || p.id === a.place_id))
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: isLinked ? 'var(--bg-hover)' : 'none',
                    border: 'none',
                    cursor: 'pointer',
                    fontSize: 13,
                    color: 'var(--text-primary)',
                  ...
                  onMouseLeave={(e) =>
                    (e.currentTarget.style.background = isLinked ? 'var(--bg-hover)' : 'transparent')
                  }
                >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {unassigned.length > 0 && (
                  <div>
                    {dayGroups.length > 0 && (
                      <div
                        style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', padding: '8px 10px 2px' }}
                      >
                        {t('files.unassigned')}
                      </div>
                    )}
                  ...
                  <>
                    <div
                      style={{
                        ...
                        textTransform: 'uppercase',
                        letterSpacing: 0.5,
                        marginTop: bookingReservations.length > 0 ? 4 : 0,
                      }}
                    >
            ...
            return (
              <div className={hasBoth ? 'md:flex' : ''}>
                <div
                  className={hasBoth ? 'md:w-1/2' : ''}
                  style={{ overflowY: 'auto', maxHeight: '55vh', paddingRight: hasBoth ? 6 : 0 }}
                >
                  {placesSection}
                </div>
                {hasBoth && (
                  <div
                    ...
                    style={{ width: 1, background: 'var(--border-primary)', flexShrink: 0 }}
                  />
                )}
                {hasBoth && (
                  <div
                    className="block md:hidden"
                    style={{ height: 1, background: 'var(--border-primary)', margin: '8px 0' }}
                  />
                )}
                <div
                  className={hasBoth ? 'md:w-1/2' : ''}
                  style={{ overflowY: 'auto', maxHeight: '55vh', paddingLeft: hasBoth ? 6 : 0 }}
                >
                  {bookingsSection}
                </div>
              </div>
