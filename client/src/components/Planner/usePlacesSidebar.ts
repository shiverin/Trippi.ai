import { CalendarDays, ExternalLink, Navigation, Pencil, Trash2 } from 'lucide-react';
...
import type { AssignmentsMap, Category, Day, Place } from '../../types';
...
export function usePlacesSidebar(props: PlacesSidebarProps) {
  ...
  const availableListImportProviders: Array<'google' | 'naver'> = isNaverListImportEnabled
    ? ['google', 'naver']
    : ['google'];
  ...
  const handleListImport = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.error || t(provider === 'google' ? 'places.googleListError' : 'places.naverListError')
      );
    } finally {
  ...
  const [filter, setFilter] = useState<'all' | 'planned' | 'unplanned' | 'tracks'>('all');
  ...
  const plannedIds = useMemo(
    () => new Set(Object.values(assignments).flatMap((da) => da.map((a) => a.place?.id ?? a.place_id).filter(Boolean))),
    [assignments]
  );
  ...
  const inDaySet = useMemo(() => {
    ...
    return new Set<number>(
      (assignments[String(selectedDayId)] || []).map((a) => a.place?.id ?? a.place_id).filter(Boolean)
    );
  }, [assignments, selectedDayId]);

  const openContextMenu = useCallback(
    (e: React.MouseEvent, place: Place) => {
      ...
      ctxMenu.open(e, [
        ...
        selDayId && {
          label: t('planner.addToDay'),
          icon: CalendarDays,
          onClick: () => props.onAssignToDay(place.id, selDayId),
        },
        place.website && {
        ...
        canEditPlaces && {
          ...
          icon: Trash2,
          danger: true,
          onClick: () => props.onDeletePlace(place.id),
        },
      ]);
    },
