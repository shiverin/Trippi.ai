...
describe('BudgetController (parity with the legacy /api/trips/:tripId/budget route)', () => {
  it('404 when the trip is not accessible', async () => {
    ...
    expect(await thrown(() => new BudgetController(svc).list(user, '5'))).toEqual({
  ...
  it('GET /summary/per-person + /settlement delegate', async () => {
    ...
    expect(await new BudgetController(svc).settlement(user, '5')).toEqual({ transfers: [] });
  ...
  it('GET /settlement forwards the base query and the trip currency', async () => {
    ...
    await new BudgetController(svc).settlement(user, '5', 'GBP');
  ...
  describe('settlements ledger', () => {
    ...
    it('POST /settlements 403 without budget_edit', async () => {
      ...
      expect(await thrown(() => new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, to_user_id: 2, amount: 10 }))).toEqual({
    ...
    it('POST /settlements 400 when a field is missing', async () => {
      const svc = makeService();
      expect(await thrown(() => new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, to_user_id: 2 }))).toEqual({
        status: 400, body: { error: 'from_user_id, to_user_id and amount are required' },
      });
      expect(await thrown(() => new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, amount: 5 }))).toEqual({
        status: 400, body: { error: 'from_user_id, to_user_id and amount are required' },
      });
      expect(await thrown(() => new BudgetController(svc).createSettlement(user, '5', { to_user_id: 2, amount: 5 }))).toEqual({
        status: 400, body: { error: 'from_user_id, to_user_id and amount are required' },
      });
    });

    it('POST /settlements creates and broadcasts (amount 0 is allowed)', async () => {
      ...
      const res = await new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, to_user_id: 2, amount: 0 }, 'sock');
    ...
    it('DELETE /settlements/:id 404 when missing', async () => {
      ...
      expect(await thrown(() => new BudgetController(svc).deleteSettlement(user, '5', '7'))).toEqual({
    ...
    it('PUT /settlements/:id 403 without budget_edit', async () => {
      const svc = makeService({ canEdit: vi.fn().mockReturnValue(false) });
      expect(await thrown(() => new BudgetController(svc).updateSettlement(user, '5', '7', { from_user_id: 1, to_user_id: 2, amount: 10 }))).toEqual({
        status: 403, body: { error: 'No permission' },
      });
    });
    ...
    it('PUT /settlements/:id 400 when a field is missing', async () => {
      const svc = makeService();
      expect(await thrown(() => new BudgetController(svc).updateSettlement(user, '5', '7', { from_user_id: 1, to_user_id: 2 }))).toEqual({
        status: 400, body: { error: 'from_user_id, to_user_id and amount are required' },
      });
    });
    ...
    it('PUT /settlements/:id updates and broadcasts', async () => {
      ...
      const res = await new BudgetController(svc).updateSettlement(user, '5', '7', { from_user_id: 2, to_user_id: 1, amount: 15 }, 'sock');
  ...
  describe('PUT /:id/members', () => {
    it('400 when user_ids is not an array', async () => {
      expect(await thrown(() => new BudgetController(makeService()).updateMembers(user, '5', '9', 'nope'))).toEqual({
    ...
    it('404 when the item is missing', async () => {
      ...
      expect(await thrown(() => new BudgetController(svc).updateMembers(user, '5', '9', [2, 3]))).toEqual({
        status: 404, body: { error: 'Budget item not found' },
      });
    });
    ...
    it('updates members and broadcasts persons count', async () => {
      ...
      const res = await new BudgetController(svc).updateMembers(user, '5', '9', [2], 'sock');
  ...
  describe('PUT /:id/payers', () => {
    it('400 when payers is not an array', async () => {
      expect(await thrown(() => new BudgetController(makeService()).setPayers(user, '5', '9', 'nope'))).toEqual({
    ...
    it('404 when the item is missing', async () => {
      ...
      expect(await thrown(() => new BudgetController(svc).setPayers(user, '5', '9', [{ user_id: 2, amount: 10 }]))).toEqual({
        status: 404, body: { error: 'Budget item not found' },
      });
    });
    ...
    it('sets payers and broadcasts budget:updated', async () => {
      ...
      const res = await new BudgetController(svc).setPayers(user, '5', '9', [{ user_id: 2, amount: 10 }], 'sock');
  ...
  it('PUT /:id/members/:userId/paid toggles + broadcasts normalised paid flag', async () => {
    ...
    expect(await new BudgetController(svc).toggleMemberPaid(user, '5', '9', '2', true, 'sock')).toEqual({ member: { user_id: 2, paid: 1 } });
  ...
  it('PUT /:id/members/:userId/paid broadcasts paid: 0 when toggled off', async () => {
    ...
    const broadcast = vi.fn();
    const svc = makeService({ toggleMemberPaid, broadcast } as Partial<BudgetService>);
    await new BudgetController(svc).toggleMemberPaid(user, '5', '9', '2', false, 'sock');
  ...
  it('DELETE /:id 404 when missing, success otherwise', async () => {
    ...
    expect(await thrown(() => new BudgetController(missing).remove(user, '5', '9'))).toEqual({
      status: 404, body: { error: 'Budget item not found' },
    });
  ...
  it('PUT /reorder/items + /reorder/categories broadcast budget:reordered', async () => {
    ...
    expect(await new BudgetController(svc).reorderItems(user, '5', [3, 1], 'sock')).toEqual({ success: true });
