...
const ru: NotificationLocale = {
  email: {
    footer: 'Вы получили это, потому что у вас включены уведомления в trippi.ai.',
    manage: 'Управление настройками',
    madeWith: 'Made with',
    openTrippi: 'Открыть trippi.ai',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} приглашает вас объединить планы отпуска. Откройте trippi.ai для подтверждения.`,
    }),
    photos_shared: (p) => ({
      title: `${p.count} фото`,
    ...
    version_available: (p) => ({
      title: 'Доступна новая версия trippi.ai',
      body: `trippi.ai ${p.version} теперь доступен. Перейдите в панель администратора для обновления.`,
    }),
  ...
  passwordReset: {
    ...
    body: 'Мы получили запрос на сброс пароля вашего аккаунта trippi.ai. Нажмите кнопку ниже, чтобы установить новый пароль.',
