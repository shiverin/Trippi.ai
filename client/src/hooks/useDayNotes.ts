...
import type { DayNote, DayNotesMap, MergedItem } from '../types';
...
export function useDayNotes(tripId: number | string) {
  ...
  const openAddNote = (
    dayId: number,
    getMergedItems: (dayId: number) => MergedItem[],
    expandDay?: (dayId: number) => void
  ) => {
    const merged = getMergedItems(dayId);
    const maxKey = merged.length > 0 ? Math.max(...merged.map((i) => i.sortKey)) : -1;
    setNoteUi((prev) => ({
      ...prev,
      [dayId]: { mode: 'add', text: '', time: '', icon: 'FileText', sortOrder: maxKey + 1 },
    }));
    expandDay?.(dayId);
  ...
  const openEditNote = (dayId: number, note: DayNote) => {
    setNoteUi((prev) => ({
      ...prev,
      [dayId]: { mode: 'edit', noteId: note.id, text: note.text, time: note.time || '', icon: note.icon || 'FileText' },
    }));
    setTimeout(() => noteInputRef.current?.focus(), 50);
  };
  ...
  const saveNote = async (dayId: number) => {
    ...
    try {
      if (ui.mode === 'add') {
        await tripStore.addDayNote(tripId, dayId, {
          text: ui.text.trim(),
          time: ui.time || null,
          icon: ui.icon || 'FileText',
          sort_order: ui.sortOrder,
        });
      } else {
        await tripStore.updateDayNote(tripId, dayId, ui.noteId!, {
          text: ui.text.trim(),
          time: ui.time || null,
          icon: ui.icon || 'FileText',
        });
  ...
  const moveNote = async (
    ...
    if (direction === 'up') {
      ...
      newSortOrder =
        idx < merged.length - 2 ? (merged[idx + 1].sortKey + merged[idx + 2].sortKey) / 2 : merged[idx + 1].sortKey + 1;
    }
    try {
