...
import { calcPD, calcPP, calcPPD, fmtNum, widgetTheme } from './BudgetPanel.helpers';
...
export function useBudgetPanel(tripId: number, tripMembers: TripMember[]) {
  ...
  const handleAddItem = async (category: string, data: AddItemData) => {
    try {
  ...
  const handleUpdateField = async (id: number, field: string, value: unknown) => {
    try {
      await updateBudgetItem(tripId, id, { [field]: value } as Partial<BudgetItem>);
    } catch {
      toast.error(t('common.error'));
    }
  };
