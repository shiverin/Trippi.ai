    ...
    <div className="space-y-5">
      ...
      {plan.block_weekends !== false && (
        <div data-testid="weekend-days" style={{ paddingLeft: 36 }}>
          <p className="mb-2 text-xs font-medium text-content-muted">{t('vacay.weekendDays')}</p>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p className="mt-0.5 text-xs text-content-faint">{t('vacay.weekStartHint')}</p>
          </div>
        </div>
      ...
      {/* Dissolve fusion */}
      {isFused && (
        <div className="mt-2 border-t border-edge-secondary pt-4">
          <div className="overflow-hidden rounded-xl border border-[rgba(239,68,68,0.2)]">
            <div className="flex items-center gap-3 bg-[rgba(239,68,68,0.06)] px-4 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-t-[rgba(239,68,68,0.1)] px-4 py-3">
              {users.map((u) => (
                <div key={u.id} className="flex items-center gap-1.5 rounded-md bg-surface-secondary px-2 py-1">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: u.color || '#6366f1' }} />
    ...
    <div className="flex items-start gap-3 rounded-xl bg-surface-secondary p-3">
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <input
          ...
          onBlur={() => {
            const v = localLabel.trim() || null;
            if (v !== cal.label) onUpdate({ label: v });
          }}
          onKeyDown={(e) => {
