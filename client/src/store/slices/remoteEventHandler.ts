 ...
 */
function writeToDexie(type: string, payload: Record<string, unknown>, state: TripStoreState): void {
  (async () => {
    try {
      switch (type) {
        // ── Places ──────────────────────────────────────────────────────────
...
export function handleRemoteEvent(set: SetState, get: GetState, event: WebSocketEvent): void {
    ...
    switch (type) {
        ...
        return {
          ...
          assignments: Object.fromEntries(
            Object.entries(state.assignments).map(([dayId, items]) => [
              dayId,
              items.map((a) =>
                a.place?.id === (payload.place as Place).id ? { ...a, place: payload.place as Place } : a
              ),
            ])
          ),
        };
      ...
      case 'assignment:moved': {
        ...
        return {
          assignments: {
            ...
            [newKey]: [
              ...(state.assignments[newKey] || []).filter((a) => a.id !== movedAssignment.id),
              movedAssignment,
            ],
          },
        };
      ...
      case 'dayNote:updated': {
        ...
        return {
          dayNotes: {
            ...state.dayNotes,
            [dayKey]: (state.dayNotes[dayKey] || []).map((n) =>
              n.id === (payload.note as DayNote).id ? (payload.note as DayNote) : n
            ),
          },
        };
      }
      ...
      case 'packing:updated':
        return {
          packingItems: state.packingItems.map((i) =>
            i.id === (payload.item as PackingItem).id ? (payload.item as PackingItem) : i
          ),
        };
      case 'packing:deleted':
        return {
      ...
      case 'todo:updated':
        return {
          todoItems: state.todoItems.map((i) =>
            i.id === (payload.item as TodoItem).id ? (payload.item as TodoItem) : i
          ),
        };
      case 'todo:deleted':
        return {
      ...
      case 'budget:updated':
        return {
          budgetItems: state.budgetItems.map((i) =>
            i.id === (payload.item as BudgetItem).id ? (payload.item as BudgetItem) : i
          ),
        };
      case 'budget:deleted':
        return {
      ...
      case 'budget:members-updated':
        return {
          budgetItems: state.budgetItems.map((i) =>
            i.id === payload.itemId
              ? { ...i, members: payload.members as BudgetItemMember[], persons: payload.persons as number }
              : i
          ),
        };
      case 'budget:member-paid-updated':
              ...
              ? // `paid` arrives over the wire as the raw value the server emits;
                ...
                // narrow without changing the value.
                {
                  ...
                  members: (i.members || []).map((m) =>
                    m.user_id === payload.userId ? { ...m, paid: payload.paid as number } : m
                  ),
                }
              : i
          ),
      ...
      case 'reservation:updated':
        return {
          reservations: state.reservations.map((r) =>
            r.id === (payload.reservation as Reservation).id ? (payload.reservation as Reservation) : r
          ),
        };
      case 'reservation:deleted':
        return {
  ...
  if (type === 'itinerary:imported') {
    const tripId = (payload.tripId as number | undefined) ?? get().trip?.id;
    if (tripId) {
      get().hydrateActiveTrip(tripId);
    }
  }
