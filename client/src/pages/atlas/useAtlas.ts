...
import { A2_TO_A3, type AtlasData, type BucketItem, type CountryDetail } from './atlasModel';

const ATLAS_CACHE_VERSION = 'v1';
const ATLAS_USER_CACHE_TTL_MS = 5 * 60 * 1000;
const ATLAS_GEO_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;

interface AtlasCacheRecord<T> {
  savedAt: number;
  data: T;
}

function atlasCacheKey(scope: string, key: string): string {
  return `trippi:atlas:${ATLAS_CACHE_VERSION}:${scope}:${key}`;
}

function readAtlasCache<T>(scope: string, key: string, ttlMs: number): T | null {
  if (typeof window === 'undefined') return null;
  try {
    const raw = window.localStorage.getItem(atlasCacheKey(scope, key));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as AtlasCacheRecord<T>;
...
function writeAtlasCache<T>(scope: string, key: string, data: T): void {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(
      atlasCacheKey(scope, key),
      JSON.stringify({ savedAt: Date.now(), data } satisfies AtlasCacheRecord<T>)
    );
  } catch {
    // localStorage can be disabled or quota-limited; cache misses are harmless.
  }
}
...
export function useAtlas() {
  ...
  const atlasCacheScope = String(useAuthStore((s) => s.user?.id ?? 'anonymous'));
  ...
  const applyCountryGeo = useCallback((geo: GeoJsonFeatureCollection): void => {
  ...
  useEffect(() => {
    let cancelled = false;
    const cachedGeo = readAtlasCache<GeoJsonFeatureCollection>('public', 'countries-geo', ATLAS_GEO_CACHE_TTL_MS);
    if (cachedGeo) applyCountryGeo(cachedGeo);
    ...
    apiClient
      ...
      .then((res) => {
        if (cancelled) return;
        const geo = res.data;
        writeAtlasCache('public', 'countries-geo', geo);
        applyCountryGeo(geo);
      })
      .catch(() => {});
    return () => {
  ...
  useEffect(() => {
    ...
    const colorMap = {};
    visitedA3List.forEach((a3, i) => {
      colorMap[a3] = VISITED_COLORS[i % VISITED_COLORS.length];
    });
  ...
  useEffect(() => {
    ...
    countryA3Set.forEach((a3, i) => {
      countryColorMap[a3] = VISITED_COLORS[i % VISITED_COLORS.length];
    });
    // Map country A2 code to country color
    const a2ColorMap: Record<string, string> = {};
    if (data)
      data.countries.forEach((c) => {
        if (A2_TO_A3[c.code] && countryColorMap[A2_TO_A3[c.code]])
          a2ColorMap[c.code] = countryColorMap[A2_TO_A3[c.code]];
      });
      ...
      mergedGeo as any,
      {
        ...
        onEachFeature: (feature, layer) => {
          ...
          const count =
            ...
            regionPlaceCounts[`${countryA2}:${regionName.toLowerCase()}`] ||
            regionPlaceCounts[`${countryA2}:${regionNameEn.toLowerCase()}`] ||
            0;
          layer.on('click', () => {
            if (!countryA2) return;
            if (visited) {
              const regionEntry = visitedRegions[countryA2]?.find(
                (r) => r.code === regionCode || r.name.toLowerCase() === regionNameEn.toLowerCase()
              );
  ...
  const handleAddBucketItem = async (): Promise<void> => {
    ...
    try {
      ...
      if (bucketForm.lat && bucketForm.lng) {
        data.lat = parseFloat(bucketForm.lat);
        data.lng = parseFloat(bucketForm.lng);
      }
      const targetDate =
        bucketForm.target_date ||
        (bucketPoiMonth > 0 && bucketPoiYear > 0
          ? `${bucketPoiYear}-${String(bucketPoiMonth).padStart(2, '0')}`
          : null);
