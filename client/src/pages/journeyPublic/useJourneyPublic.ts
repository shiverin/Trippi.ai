...
export function useJourneyPublic() {
  ...
  const handleMarkerClick = useCallback((entryId: string) => {
    ...
    document.querySelector(`[data-entry-id="${entryId}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, []);

  useEffect(() => {
    if (!token) return;
    journeyApi
      .getPublicJourney(token)
      .then((d) => setData(d))
      .catch(() => setError(true))
  ...
  const mapEntries = useMemo(() => timelineEntries.filter((e) => e.location_lat && e.location_lng), [timelineEntries]);
