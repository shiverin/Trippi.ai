  ...
  return (
    <PageShell className="bg-zinc-50 dark:bg-zinc-950" navOffset="var(--nav-h, 56px)">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="hidden px-8 pb-7 pt-10 md:block">
          <div
              ...
              >
                ...
                {activeJourney.cover_image && (
                  <div className="absolute inset-0 z-[1]">
                    <img src={`/uploads/${activeJourney.cover_image}`} className="h-full w-full object-cover" alt="" />
                    <div
                      className="absolute inset-0"
                      style={{ background: pickGradient(activeJourney.id), opacity: 0.45 }}
                    />
                  </div>
                )}
                ...
                <div
                  ...
                  style={{
                    background:
                      'radial-gradient(circle at 15% 20%, rgba(236,72,153,0.35), transparent 40%), radial-gradient(circle at 85% 80%, rgba(251,146,60,0.3), transparent 45%), radial-gradient(circle at 50% 50%, rgba(99,102,241,0.25), transparent 50%)',
                  }}
                />
                <div
                  className="pointer-events-none absolute inset-0 z-[2]"
                  style={{
                    background:
                      'linear-gradient(180deg, transparent 0%, transparent 50%, rgba(0,0,0,0.4) 100%), linear-gradient(90deg, rgba(0,0,0,0.15) 0%, transparent 50%)',
                  }}
                />

                <div className="relative z-[3] flex h-full flex-col p-6 text-white md:p-8">
                  {/* Top badges */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="bg-white/12 inline-flex items-center gap-2 rounded-full border border-white/15 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] backdrop-blur-sm">
                      ...
                      <span className="bg-white/12 inline-flex items-center gap-1.5 rounded-full border border-white/15 px-3 py-1.5 text-[10px] font-medium backdrop-blur-sm">
                        <RefreshCw size={10} />
                  ...
                  <div className="flex flex-1 flex-col justify-center py-4">
                    {activeJourney.subtitle && (
                      <p className="mb-3 text-[13px] font-medium opacity-85">{activeJourney.subtitle}</p>
                    )}
                    <h2
                      className="mb-3 text-[40px] font-extrabold leading-[0.95] tracking-[-0.035em] md:text-[56px]"
                  ...
                  <div className="flex items-end justify-between gap-6">
                    <div className="flex gap-7">
                      {[
                        ...
                        <div key={s.label} className="flex flex-col gap-1">
                          <span className="text-[28px] font-extrabold leading-none tracking-[-0.02em]">{s.val}</span>
                    ...
                    <span className="hidden items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-[11px] font-medium backdrop-blur-sm md:inline-flex">
            ...
            <div className="flex-1 overflow-y-auto px-7 py-5">
              ...
              <div className="flex max-h-[320px] flex-col gap-2 overflow-y-auto">
                {availableTrips.map((trip) => {
                  ...
                  const status =
                    trip.end_date && trip.end_date < new Date().toISOString().split('T')[0]
                      ? 'completed'
...
function JourneyCard({
  ...
  return (
    <div
      onClick={onClick}
      className="duration-250 flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white transition-[transform,box-shadow,border-color] ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-1 hover:border-zinc-400 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] dark:border-zinc-700 dark:bg-zinc-900"
    >
      {/* Cover */}
      <div className="relative h-[170px] overflow-hidden" style={{ background: pickGradient(j.id) }}>
        ...
        {/* Top overlay */}
        <div className="absolute left-3.5 right-3.5 top-3.5 z-[2] flex items-start justify-between">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-black/45 px-2.5 py-1 text-[10px] font-semibold tracking-wide text-white backdrop-blur-sm">
            <Calendar size={10} />
      ...
      <div className="flex flex-1 flex-col px-[18px] pb-[18px] pt-4">
        ...
        {j.subtitle && <p className="mt-1 text-[12px] text-zinc-500">{j.subtitle}</p>}
        {lifecycle !== 'live' && (
          <span
            className={`mt-1.5 inline-flex self-start rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${
              lifecycle === 'archived'
        ...
        <div
          className="mt-auto grid grid-cols-3 gap-2.5 border-t border-zinc-100 pt-3.5 dark:border-zinc-800"
          style={{ marginTop: j.subtitle ? 14 : 'auto' }}
        >
          {[
            ...
            <div key={s.label} className="flex flex-col gap-1">
              <span
                className={`text-[16px] font-bold leading-none tracking-[-0.01em] ${s.val > 0 ? 'text-zinc-900 dark:text-white' : 'text-zinc-300 dark:text-zinc-600'}`}
              >
                {s.val > 0 ? s.val : '--'}
              </span>
              <span className="text-[9px] font-medium uppercase tracking-[0.06em] text-zinc-500">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
