...
const MIDNIGHT_WRAP_THRESHOLD_MINUTES = 12 * 60;
...
function applyChronoOrder(
  ...
  return items
    .map((it, i) => {
      const t = timeOf(it);
      if (t != null) {
        const adjusted = lastTimed != null && t + MIDNIGHT_WRAP_THRESHOLD_MINUTES < lastTimed ? t + 24 * 60 : t;
        lastTimed = adjusted;
        last = adjusted;
        return { it, i, eff: adjusted };
      }
      return { it, i, eff: last };
    })
...
export function getMergedItems(opts: {
  ...
  if (baseItems.length === 0) {
    return applyChronoOrder(
      timedTransports.map((item, i) => ({ type: item.type, sortKey: i, data: item.data })),
      dayId,
      getDisplayTime
    );
  }
  ...
  for (let ti = 0; ti < timedTransports.length; ti++) {
    ...
    const sortKey = insertAfterKey === -Infinity ? lastKey + 0.5 + ti * 0.01 : insertAfterKey + 0.01 + ti * 0.001;
