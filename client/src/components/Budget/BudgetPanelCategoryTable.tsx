...
import { calcPD, calcPP, calcPPD } from './BudgetPanel.helpers';
...
import type { AddItemData, EditingCat } from './useBudgetPanel';
...
interface BudgetCategoryTableProps {
  ...
  setBudgetItemMembers: (
    ...
    itemId: number,
    userIds: number[]
  ) => Promise<{ members: unknown; item: unknown }>;
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
          {canEdit && (
            <div
              ...
              onDragStart={(e) => {
                ...
                e.dataTransfer.setData('text/x-budget-cat', cat);
                setDragCat(cat);
              }}
              onDragEnd={() => {
                setDragCat(null);
          ...
          {canEdit && editingCat?.name === cat ? (
            <input
              ...
              onBlur={() => {
                handleRenameCategory(cat, editingCat.value);
                setEditingCat(null);
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              <th className="hidden sm:table-cell" style={{ ...th, minWidth: 160 }}>
                {t('budget.table.persons')}
              </th>
              <th className="hidden sm:table-cell" style={{ ...th, minWidth: 55 }}>
                {t('budget.table.days')}
              </th>
              <th className="hidden md:table-cell" style={{ ...th, minWidth: 100 }}>
                {t('budget.table.perPerson')}
              </th>
              <th className="hidden md:table-cell" style={{ ...th, minWidth: 90 }}>
                {t('budget.table.perDay')}
              </th>
              <th className="hidden lg:table-cell" style={{ ...th, minWidth: 95 }}>
                {t('budget.table.perPersonDay')}
              </th>
              <th className="hidden sm:table-cell" style={{ ...th, width: 90, maxWidth: 90 }}>
                {t('budget.table.date')}
              </th>
              <th className="hidden sm:table-cell" style={{ ...th, minWidth: 150 }}>
                {t('budget.table.note')}
              </th>
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <InlineEditCell
                          ...
                          placeholder={t('budget.table.name')}
                          locale={locale}
                          editTooltip={item.reservation_id ? t('budget.linkedToReservation') : t('budget.editTooltip')}
                          readOnly={!canEdit || !!item.reservation_id}
                        />
                        {hasMultipleMembers && (
                  ...
                  <td style={{ ...td, textAlign: 'center' }}>
                    <InlineEditCell
                      ...
                      onSave={(v) => handleUpdateField(item.id, 'total_price', v)}
                  ...
                  <td className="hidden sm:table-cell" style={{ ...td, textAlign: 'center', position: 'relative' }}>
                    {hasMultipleMembers ? (
                      ...
                      <InlineEditCell
                        ...
                        decimals={0}
                        onSave={(v) =>
                          handleUpdateField(item.id, 'persons', v != null ? parseInt(v as string) || null : null)
                        }
                        style={{ textAlign: 'center' }}
                        placeholder="-"
                        locale={locale}
                  ...
                  <td className="hidden sm:table-cell" style={{ ...td, textAlign: 'center' }}>
                    <InlineEditCell
                      ...
                      decimals={0}
                      onSave={(v) =>
                        handleUpdateField(item.id, 'days', v != null ? parseInt(v as string) || null : null)
                      }
                      style={{ textAlign: 'center' }}
                      placeholder="-"
                      locale={locale}
                  ...
                  <td
                    ...
                    style={{
                      ...td,
                      textAlign: 'center',
                      color: pp != null ? 'var(--text-secondary)' : 'var(--text-faint)',
                    }}
                  >
                  ...
                  <td
                    ...
                    style={{
                      ...td,
                      textAlign: 'center',
                      color: pd != null ? 'var(--text-secondary)' : 'var(--text-faint)',
                    }}
                  >
                  ...
                  <td
                    ...
                    style={{
                      ...td,
                      textAlign: 'center',
                      color: ppd != null ? 'var(--text-secondary)' : 'var(--text-faint)',
                    }}
                  >
                  ...
                  <td
                    ...
                    style={{ ...td, padding: '2px 6px', width: 90, maxWidth: 90, textAlign: 'center' }}
                  >
                    {canEdit ? (
                      <div style={{ maxWidth: 90, margin: '0 auto' }}>
                        <CustomDatePicker
                          value={item.expense_date || ''}
                          onChange={(v) => handleUpdateField(item.id, 'expense_date', v || null)}
                          placeholder="—"
                          compact
                          borderless
                        />
                      ...
                      <span
                        style={{
                          fontSize: 11,
                          color: item.expense_date ? 'var(--text-secondary)' : 'var(--text-faint)',
                        }}
                      >
                        {item.expense_date || '—'}
                      </span>
                  ...
                  <td className="hidden sm:table-cell" style={td}>
                    <InlineEditCell
                      value={item.note}
                      onSave={(v) => handleUpdateField(item.id, 'note', v)}
