...
export function FilesView(S: FileManagerState) {
  ...
  return (
    <>
      ...
      {can('file_upload', trip) && (
        ...
        >
          ...
          <Upload
            ...
            style={{
              margin: '0 auto 8px',
              color: isDragActive ? 'var(--text-secondary)' : 'var(--text-faint)',
              display: 'block',
            }}
          />
          {uploading ? (
            ...
            <>
              ...
              <p style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 6, opacity: 0.7 }}>
                {(allowedFileTypes || 'jpg,jpeg,png,gif,webp,heic,pdf,doc,docx,xls,xlsx,txt,csv')
                  .toUpperCase()
                  .split(',')
                  .join(', ')}{' '}
                · Max 50 MB
      ...
      <div
        className="md:!hidden"
        style={{ display: 'flex', gap: 4, padding: '12px 16px 0', flexShrink: 0, flexWrap: 'wrap' }}
      >
        {[
          ...
          >
            {tab.icon ? (
              <tab.icon
                size={13}
                fill={filterType === tab.id ? '#facc15' : 'none'}
                color={filterType === tab.id ? '#facc15' : 'currentColor'}
              />
            ) : (
              tab.label
            )}
        ...
        {filteredFiles.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filteredFiles.map((file) => (
              <FileRow key={file.id} {...S} file={file} />
            ))}
          </div>
        )}
      </div>
