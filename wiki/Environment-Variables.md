...
> Use this reference only for non-public infrastructure, development, or maintenance deployments.
...
- **Proxmox Community Script** — set in `/opt/trippi/server/.env`
...
| `SESSION_DURATION`          | How long a login session stays valid before re-login is required. Used when **"Remember me" is unchecked** on the login form (the default): applies to the `trippi_session` JWT `exp` claim, and the cookie is issued as a **browser-session cookie** (no `maxAge`, cleared when the browser closes). Accepts `ms`-style strings: `1h`, `12h`, `7d`, `30d`, `90d`. Invalid values warn at startup and fall back to the default. Does not affect the short-lived MFA challenge token or MCP OAuth tokens (those keep their own TTL). | `24h`                           |
| `SESSION_DURATION_REMEMBER` | Session length used when the user **ticks "Remember me"** on login: a longer-lived JWT `exp` claim plus a **persistent** `trippi_session` cookie whose `maxAge` matches, so the session survives browser restarts. Same `ms`-style format and startup-fallback behaviour as `SESSION_DURATION`.                                                                                                                                                                                                                                     | `30d`                           |
...
| `APP_URL`                   | Public base URL (e.g. `https://trippi.example.com`). Required when OIDC is enabled — must match the redirect URI registered with your IdP. Also used as the base URL for email notification links.                                                                                                                                                                                                                                                                                                                                  | —                               |
...
| `COOKIE_SECURE`           | Controls the `secure` flag on the `trippi_session` cookie. Auto-derived as `true` when `NODE_ENV=production` or `FORCE_HTTPS=true`. Set to `false` only as an escape hatch for LAN testing without TLS — not recommended in production.                                                 | auto             |
...
| `OIDC_ADMIN_VALUE`   | Value of the OIDC claim that grants admin role (e.g. `app-trippi-admins`)                                                                                                              | —                      |
...
| Variable           | Description                                                                                                                                                                                                                                                                                                                                                                     | Default                |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `WEBAUTHN_RP_ID`   | Relying-Party ID — the registrable domain passkeys are bound to (e.g. `trippi.example.com`). Overrides the `webauthn_rp_id` DB setting. When unset, it is derived from the hostname of `APP_URL`. Bare IP literals (IPv4/IPv6) are rejected. If it cannot be resolved, passkeys are disabled.                                                                                   | derived from `APP_URL` |
| `WEBAUTHN_ORIGINS` | Comma-separated list of allowed origins for passkey ceremonies (e.g. `https://trippi.example.com`). Overrides the `webauthn_origins` DB setting; trailing slashes are stripped. When unset and the RP ID is not `localhost`, a single origin is derived from `APP_URL`. In dev (RP ID `localhost`) `http://localhost:5173` and `http://localhost:3001` are added automatically. | derived from `APP_URL` |
...
| Variable         | Description                          | Default              |
| ---------------- | ------------------------------------ | -------------------- |
| `ADMIN_EMAIL`    | Email for the first admin account    | `admin@trippi.local` |
| `ADMIN_PASSWORD` | Password for the first admin account | random               |

Both variables must be set together. If either is omitted, the account is created with email `admin@trippi.local` and a
...
| Variable                              | Description                                                                                                                                                                                                                                                                                | Default                 |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| `TRIPPI_MEDIA_BACKEND`                | Media storage backend for uploaded avatars, covers, journey photos, exported PDFs, and file attachments. Use `gcs` in production so user media is not tied to one VM filesystem.                                                                                                           | `local`                 |
| `TRIPPI_GCS_BUCKET`                   | Required when `TRIPPI_MEDIA_BACKEND=gcs`. Cloud Storage bucket for uploaded media. Use public access prevention and grant the app service account bucket-level object access.                                                                                                              | unset                   |
| `TRIPPI_GCS_PREFIX`                   | Object key prefix inside the GCS bucket. Use a stable prefix such as `prod` so staging/prod media can share a bucket safely if needed.                                                                                                                                                     | `prod`                  |
| `TRIPPI_MEDIA_DELIVERY`               | `backend` streams media through Node. `signed-url` authorizes through the app, then redirects to a short-lived GCS signed URL so media bytes are served by Cloud Storage. `signed-url` requires `TRIPPI_MEDIA_BACKEND=gcs`.                                                                | `backend`               |
| `TRIPPI_MEDIA_SIGNED_URL_TTL_SECONDS` | Lifetime for generated GCS signed read URLs. Keep short for private media; browsers can always re-request the app URL to receive a fresh signed URL.                                                                                                                                       | `300`                   |
| `TRIPPI_PLACE_PHOTO_DIR`              | Legacy/local directory for cached Google place photos. In GCS media mode, new place-photo cache writes go through the configured media backend.                                                                                                                                            | `uploads/photos/google` |
| `BACKUP_UPLOAD_LIMIT_MB`              | Maximum **compressed** size (in MB) of a restore-backup archive that may be uploaded. Raise it if your backups exceed the default. In GCS media mode, backup ZIPs contain a media manifest; protect the bucket separately with object versioning/lifecycle or bucket-level backup tooling. | `500`                   |
...
| `RATE_LIMIT_MAX_KEYS_PER_BUCKET` | Maximum in-memory auth/OAuth limiter keys retained per bucket before expired and oldest keys are pruned. Raises memory headroom for very large single-instance deployments; app-level limits are still per process.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `100000`               |
| `OVERPASS_URL`                   | Custom [Overpass API](https://wiki.openstreetmap.org/wiki/Overpass_API) endpoint(s) used by the map's POI "explore" search, comma-separated. When set it **replaces** the bundled public mirrors — point it at an internal or private Overpass instance when the public mirrors are unreachable from your network (e.g. firewalled/locked-down egress in a Kubernetes cluster). Entries that aren't valid `http(s)` URLs are ignored. If you don't operate a private Overpass endpoint but the public mirrors throttle trippi.ai, first make sure `APP_URL` (or `ALLOWED_ORIGINS`) is set: that alone gives outbound Overpass/Nominatim requests a unique User-Agent, which the public mirrors rate-limit far less. | bundled public mirrors |
| `OVERPASS_TIMEOUT_MS`            | Per-endpoint timeout (in milliseconds) for Overpass POI requests. Endpoints race in parallel and one that hasn't answered within this window is abandoned so a faster mirror can win. Raise it if you use a slow private Overpass instance. Invalid values fall back to the default.                                                                                                                                                                                                                                                                                                                                                                                                                       | `12000`                |
...
| `DEMO_ADMIN_EMAIL` | Email of the seeded demo admin account.                                                                                                                                                                                                                                     | `admin@trippi.app` |
