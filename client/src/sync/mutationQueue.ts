...
export const mutationQueue = {
   ...
   */
  async enqueue(mutation: Omit<QueuedMutation, 'status' | 'attempts' | 'createdAt' | 'lastError'>): Promise<string> {
    const now = Date.now();
  ...
  async pending(tripId?: number): Promise<QueuedMutation[]> {
    ...
    return offlineDb.mutationQueue.where('status').anyOf(['pending', 'syncing']).toArray();
  },

  /** Count pending mutations (for banner badge). */
  async pendingCount(): Promise<number> {
    return offlineDb.mutationQueue.where('status').anyOf(['pending', 'syncing']).count();
  },
  ...
  async failedCount(): Promise<number> {
    return offlineDb.mutationQueue.where('status').equals('failed').count();
  },
