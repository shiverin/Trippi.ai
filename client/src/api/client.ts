import {
  ...
  type CollabNoteUpdateRequest,
  type CollabPollCreateRequest,
  type CollabPollVoteRequest,
  type CollabReactionRequest,
  type CreateCategoryRequest,
  type CreateTagRequest,
  type DayCreateRequest,
  type DayNoteCreateRequest,
  type DayNoteUpdateRequest,
  type DayReorderRequest,
  ...
  type JourneyProviderPhotosRequest,
  type JourneyReorderEntriesRequest,
...
export const API_TIMEOUTS = {
  ...
  background: parseTimeout(
    (import.meta as { env?: { VITE_API_BACKGROUND_TIMEOUT_MS?: string } }).env?.VITE_API_BACKGROUND_TIMEOUT_MS,
    15000
  ),
  heavy: parseTimeout(
    (import.meta as { env?: { VITE_API_HEAVY_TIMEOUT_MS?: string } }).env?.VITE_API_HEAVY_TIMEOUT_MS,
    45000
  ),
};
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      (error.response?.data as { code?: string } | undefined)?.code === 'AUTH_REQUIRED'
    ) {
      const { pathname } = window.location;
...
export const authApi = {
  ...
  mfaEnable: (data: MfaEnableRequest) =>
    apiClient
      .post('/auth/mfa/enable', data)
      .then((r) => r.data as { success: boolean; mfa_enabled: boolean; backup_codes?: string[] }),
  mfaDisable: (data: { password: string; code: string }) =>
    apiClient.post('/auth/mfa/disable', data).then((r) => r.data),
  ...
  mcpTokens: {
    ...
    create: (name: string) =>
      apiClient.post('/auth/mcp-tokens', { name } satisfies McpTokenCreateRequest).then((r) => r.data),
  ...
  passkey: {
    ...
    rename: (id: number, name: string) =>
      apiClient.patch(`/auth/passkey/credentials/${id}`, { name }).then((r) => r.data),
...
export const tripsApi = {
  ...
  addMember: (id: number | string, identifier: string) =>
    apiClient.post(`/trips/${id}/members`, { identifier } satisfies TripAddMemberRequest).then((r) => r.data),
  removeMember: (id: number | string, userId: number) =>
    apiClient.delete(`/trips/${id}/members/${userId}`).then((r) => r.data),
...
export const placesApi = {
  list: (tripId: number | string, params?: Record<string, unknown>) =>
    apiClient.get(`/trips/${tripId}/places`, { params }).then((r) => r.data),
  create: (tripId: number | string, data: PlaceCreateRequest) =>
    apiClient.post(`/trips/${tripId}/places`, data).then((r) => r.data),
  get: (tripId: number | string, id: number | string) =>
    apiClient.get(`/trips/${tripId}/places/${id}`).then((r) => r.data),
  update: (tripId: number | string, id: number | string, data: PlaceUpdateRequest) =>
    apiClient.put(`/trips/${tripId}/places/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number | string) =>
    apiClient.delete(`/trips/${tripId}/places/${id}`).then((r) => r.data),
  searchImage: (tripId: number | string, id: number | string) =>
    apiClient.get(`/trips/${tripId}/places/${id}/image`).then((r) => r.data),
  importGpx: (
    tripId: number | string,
    file: File,
    opts?: { waypoints?: boolean; routes?: boolean; tracks?: boolean }
  ) => {
    ...
    return apiClient
      .post(`/trips/${tripId}/places/import/gpx`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      .then((r) => r.data);
  },
  importMapFile: (tripId: number | string, file: File, opts?: { points?: boolean; paths?: boolean }) => {
    ...
    return apiClient
      .post(`/trips/${tripId}/places/import/map`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      .then((r) => r.data);
  },
  ...
  importNaverList: (tripId: number | string, url: string, enrich?: boolean) =>
    apiClient
      .post(`/trips/${tripId}/places/import/naver-list`, { url, enrich } satisfies PlaceImportListRequest)
      .then((r) => r.data),
...
export const assignmentsApi = {
  list: (tripId: number | string, dayId: number | string) =>
    apiClient.get(`/trips/${tripId}/days/${dayId}/assignments`).then((r) => r.data),
  create: (tripId: number | string, dayId: number | string, data: AssignmentCreateRequest) =>
    apiClient.post(`/trips/${tripId}/days/${dayId}/assignments`, data).then((r) => r.data),
  delete: (tripId: number | string, dayId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/days/${dayId}/assignments/${id}`).then((r) => r.data),
  reorder: (tripId: number | string, dayId: number | string, orderedIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/days/${dayId}/assignments/reorder`, { orderedIds } satisfies AssignmentReorderRequest)
      .then((r) => r.data),
  move: (tripId: number | string, assignmentId: number, newDayId: number | string, orderIndex: number | null) =>
    apiClient
      .put(`/trips/${tripId}/assignments/${assignmentId}/move`, { new_day_id: newDayId, order_index: orderIndex })
      .then((r) => r.data),
  update: (tripId: number | string, dayId: number | string, id: number, data: Record<string, unknown>) =>
    apiClient.put(`/trips/${tripId}/days/${dayId}/assignments/${id}`, data).then((r) => r.data),
  getParticipants: (tripId: number | string, id: number) =>
    apiClient.get(`/trips/${tripId}/assignments/${id}/participants`).then((r) => r.data),
  setParticipants: (tripId: number | string, id: number, userIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/assignments/${id}/participants`, {
        user_ids: userIds,
      } satisfies AssignmentParticipantsRequest)
      .then((r) => r.data),
...
export const packingApi = {
  ...
  create: (tripId: number | string, data: PackingCreateItemRequest) =>
    apiClient.post(`/trips/${tripId}/packing`, data).then((r) => r.data),
  bulkImport: (tripId: number | string, items: { name: string; category?: string; quantity?: number }[]) =>
    apiClient.post(`/trips/${tripId}/packing/import`, { items } satisfies PackingImportRequest).then((r) => r.data),
  update: (tripId: number | string, id: number, data: PackingUpdateItemRequest) =>
    apiClient.put(`/trips/${tripId}/packing/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/packing/${id}`).then((r) => r.data),
  reorder: (tripId: number | string, orderedIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/packing/reorder`, { orderedIds } satisfies PackingReorderRequest)
      .then((r) => r.data),
  getCategoryAssignees: (tripId: number | string) =>
    apiClient.get(`/trips/${tripId}/packing/category-assignees`).then((r) => r.data),
  setCategoryAssignees: (tripId: number | string, categoryName: string, userIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/packing/category-assignees/${encodeURIComponent(categoryName)}`, {
        user_ids: userIds,
      } satisfies PackingCategoryAssigneesRequest)
      .then((r) => r.data),
  ...
  applyTemplate: (tripId: number | string, templateId: number) =>
    apiClient.post(`/trips/${tripId}/packing/apply-template/${templateId}`).then((r) => r.data),
  saveAsTemplate: (tripId: number | string, name: string) =>
    apiClient.post(`/trips/${tripId}/packing/save-as-template`, { name }).then((r) => r.data),
  setBagMembers: (tripId: number | string, bagId: number, userIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/packing/bags/${bagId}/members`, { user_ids: userIds } satisfies PackingBagMembersRequest)
      .then((r) => r.data),
  listBags: (tripId: number | string) => apiClient.get(`/trips/${tripId}/packing/bags`).then((r) => r.data),
  createBag: (tripId: number | string, data: PackingCreateBagRequest) =>
    apiClient.post(`/trips/${tripId}/packing/bags`, data).then((r) => r.data),
  updateBag: (tripId: number | string, bagId: number, data: PackingUpdateBagRequest) =>
    apiClient.put(`/trips/${tripId}/packing/bags/${bagId}`, data).then((r) => r.data),
  deleteBag: (tripId: number | string, bagId: number) =>
    apiClient.delete(`/trips/${tripId}/packing/bags/${bagId}`).then((r) => r.data),
};
...
export const todoApi = {
  ...
  create: (tripId: number | string, data: TodoCreateItemRequest) =>
    apiClient.post(`/trips/${tripId}/todo`, data).then((r) => r.data),
  update: (tripId: number | string, id: number, data: TodoUpdateItemRequest) =>
    apiClient.put(`/trips/${tripId}/todo/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number) => apiClient.delete(`/trips/${tripId}/todo/${id}`).then((r) => r.data),
  reorder: (tripId: number | string, orderedIds: number[]) =>
    apiClient.put(`/trips/${tripId}/todo/reorder`, { orderedIds } satisfies TodoReorderRequest).then((r) => r.data),
  getCategoryAssignees: (tripId: number | string) =>
    apiClient.get(`/trips/${tripId}/todo/category-assignees`).then((r) => r.data),
  setCategoryAssignees: (tripId: number | string, categoryName: string, userIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/todo/category-assignees/${encodeURIComponent(categoryName)}`, {
        user_ids: userIds,
      } satisfies TodoCategoryAssigneesRequest)
      .then((r) => r.data),
};
...
export const adminApi = {
  ...
  updateAddon: (id: number | string, data: Record<string, unknown>) =>
    apiClient.put(`/admin/addons/${id}`, data).then((r) => r.data),
  ...
  createPackingTemplate: (data: { name: string }) =>
    apiClient.post('/admin/packing-templates', data).then((r) => r.data),
  updatePackingTemplate: (id: number, data: { name: string }) =>
    apiClient.put(`/admin/packing-templates/${id}`, data).then((r) => r.data),
  deletePackingTemplate: (id: number) => apiClient.delete(`/admin/packing-templates/${id}`).then((r) => r.data),
  addTemplateCategory: (templateId: number, data: { name: string }) =>
    apiClient.post(`/admin/packing-templates/${templateId}/categories`, data).then((r) => r.data),
  updateTemplateCategory: (templateId: number, catId: number, data: { name: string }) =>
    apiClient.put(`/admin/packing-templates/${templateId}/categories/${catId}`, data).then((r) => r.data),
  deleteTemplateCategory: (templateId: number, catId: number) =>
    apiClient.delete(`/admin/packing-templates/${templateId}/categories/${catId}`).then((r) => r.data),
  addTemplateItem: (templateId: number, catId: number, data: { name: string }) =>
    apiClient.post(`/admin/packing-templates/${templateId}/categories/${catId}/items`, data).then((r) => r.data),
  updateTemplateItem: (templateId: number, itemId: number, data: { name: string }) =>
    apiClient.put(`/admin/packing-templates/${templateId}/items/${itemId}`, data).then((r) => r.data),
  deleteTemplateItem: (templateId: number, itemId: number) =>
    apiClient.delete(`/admin/packing-templates/${templateId}/items/${itemId}`).then((r) => r.data),
  listInvites: () => apiClient.get('/admin/invites').then((r) => r.data),
  createInvite: (data: { max_uses: number; expires_in_days?: number }) =>
    apiClient.post('/admin/invites', data).then((r) => r.data),
  ...
  updatePermissions: (permissions: Record<string, string>) =>
    apiClient.put('/admin/permissions', { permissions }).then((r) => r.data),
  ...
  updateNotificationPreferences: (prefs: Record<string, Record<string, boolean>>) =>
    apiClient.put('/admin/notification-preferences', prefs).then((r) => r.data),
  getDefaultUserSettings: () => apiClient.get('/admin/default-user-settings').then((r) => r.data),
  updateDefaultUserSettings: (settings: Record<string, unknown>) =>
    apiClient.put('/admin/default-user-settings', settings).then((r) => r.data),
};
...
export const journeyApi = {
  ...
  addTrip: (id: number, tripId: number) =>
    apiClient.post(`/journeys/${id}/trips`, { trip_id: tripId } satisfies JourneyAddTripRequest).then((r) => r.data),
  ...
  createEntry: (id: number, data: Record<string, unknown>) =>
    apiClient.post(`/journeys/${id}/entries`, data).then((r) => r.data),
  updateEntry: (entryId: number, data: Record<string, unknown>) =>
    apiClient.patch(`/journeys/entries/${entryId}`, data).then((r) => r.data),
  deleteEntry: (entryId: number) => apiClient.delete(`/journeys/entries/${entryId}`).then((r) => r.data),
  reorderEntries: (journeyId: number, orderedIds: number[]) =>
    apiClient
      .put(`/journeys/${journeyId}/entries/reorder`, { orderedIds } satisfies JourneyReorderEntriesRequest)
      .then((r) => r.data),
  ...
  uploadPhotos: (
    ...
    apiClient
      .post(`/journeys/entries/${entryId}/photos`, formData, {
        headers: {
  ...
  uploadGalleryPhotos: (
    ...
    apiClient
      .post(`/journeys/${journeyId}/gallery/photos`, formData, {
        headers: {
          'Content-Type': undefined as any,
          ...(opts?.idempotencyKey ? { 'X-Idempotency-Key': opts.idempotencyKey } : {}),
        },
      ...
      .then((r) => r.data),
  addProviderPhotosToGallery: (journeyId: number, provider: string, assetIds: string[], passphrase?: string) =>
    apiClient
      .post(`/journeys/${journeyId}/gallery/provider-photos`, {
        provider,
        asset_ids: assetIds,
        ...(passphrase ? { passphrase } : {}),
      } satisfies JourneyProviderPhotosRequest)
      .then((r) => r.data),
  addProviderPhoto: (entryId: number, provider: string, assetId: string, caption?: string, passphrase?: string) =>
    apiClient
      .post(`/journeys/entries/${entryId}/provider-photos`, {
        provider,
        asset_id: assetId,
        caption,
        ...(passphrase ? { passphrase } : {}),
      ...
      .then((r) => r.data),
  addProviderPhotos: (entryId: number, provider: string, assetIds: string[], caption?: string, passphrase?: string) =>
    apiClient
      .post(`/journeys/entries/${entryId}/provider-photos`, {
        provider,
        asset_ids: assetIds,
      ...
      .then((r) => r.data),
  linkPhoto: (entryId: number, journeyPhotoId: number) =>
    apiClient.post(`/journeys/entries/${entryId}/link-photo`, { journey_photo_id: journeyPhotoId }).then((r) => r.data),
  unlinkPhoto: (entryId: number, journeyPhotoId: number) =>
    apiClient.delete(`/journeys/entries/${entryId}/photos/${journeyPhotoId}`).then((r) => r.data),
  deleteGalleryPhoto: (journeyId: number, journeyPhotoId: number) =>
    apiClient.delete(`/journeys/${journeyId}/gallery/${journeyPhotoId}`).then((r) => r.data),
  updatePhoto: (photoId: number, data: Record<string, unknown>) =>
  ...
  uploadCover: (id: number, formData: FormData) =>
    apiClient
      .post(`/journeys/${id}/cover`, formData, { headers: { 'Content-Type': undefined as any } })
      .then((r) => r.data),

  // Contributors
  addContributor: (id: number, userId: number, role: string) =>
    apiClient.post(`/journeys/${id}/contributors`, { user_id: userId, role }).then((r) => r.data),
  updateContributor: (id: number, userId: number, role: string) =>
    apiClient.patch(`/journeys/${id}/contributors/${userId}`, { role }).then((r) => r.data),
  removeContributor: (id: number, userId: number) =>
    apiClient.delete(`/journeys/${id}/contributors/${userId}`).then((r) => r.data),
  ...
  createShareLink: (id: number, perms: JourneyShareLinkRequest) =>
    apiClient.post(`/journeys/${id}/share-link`, perms).then((r) => r.data),
...
export const mapsApi = {
  ...
  autocomplete: (
    ...
    apiClient
      .post('/maps/autocomplete', { input, lang, locationBias }, { signal })
      .then((r) => checkInDev(mapsAutocompleteResultSchema, r.data, 'maps.autocomplete')),
  details: (placeId: string, lang?: string) =>
    apiClient
      .get(`/maps/details/${encodeURIComponent(placeId)}`, { params: { lang } })
      .then((r) => checkInDev(mapsPlaceDetailsResultSchema, r.data, 'maps.details')),
  placePhoto: (placeId: string, lat?: number, lng?: number, name?: string) =>
    apiClient
      .get(`/maps/place-photo/${encodeURIComponent(placeId)}`, { params: { lat, lng, name } })
      .then((r) => checkInDev(mapsPlacePhotoResultSchema, r.data, 'maps.placePhoto')),
  reverse: (lat: number, lng: number, lang?: string) =>
    apiClient
      .get('/maps/reverse', { params: { lat, lng, lang } })
  ...
  transportRoute: (tripId: number | string, reservationId: number | string, signal?: AbortSignal) =>
    apiClient
      ...
      .then(
        (r) => checkInDev(mapsTransportRouteResultSchema, r.data, 'maps.transportRoute') as MapsTransportRouteResult
      ),
...
export const airportsApi = {
  search: (q: string, signal?: AbortSignal) =>
    apiClient.get('/airports/search', { params: { q }, signal }).then((r) => r.data),
...
export const budgetApi = {
  ...
  create: (tripId: number | string, data: BudgetCreateItemRequest) =>
    apiClient.post(`/trips/${tripId}/budget`, data).then((r) => r.data),
  update: (tripId: number | string, id: number, data: BudgetUpdateItemRequest) =>
    apiClient.put(`/trips/${tripId}/budget/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/budget/${id}`).then((r) => r.data),
  setMembers: (tripId: number | string, id: number, userIds: number[]) =>
    apiClient
      .put(`/trips/${tripId}/budget/${id}/members`, { user_ids: userIds } satisfies BudgetUpdateMembersRequest)
      .then((r) => r.data),
  togglePaid: (tripId: number | string, id: number, userId: number, paid: boolean) =>
    apiClient
      .put(`/trips/${tripId}/budget/${id}/members/${userId}/paid`, { paid } satisfies BudgetToggleMemberPaidRequest)
      .then((r) => r.data),
  setPayers: (tripId: number | string, id: number, payers: { user_id: number; amount: number }[]) =>
  ...
  createSettlement: (tripId: number | string, data: { from_user_id: number; to_user_id: number; amount: number }) =>
  ...
  reorderItems: (tripId: number | string, orderedIds: number[]) =>
    apiClient.put(`/trips/${tripId}/budget/reorder/items`, { orderedIds }).then((r) => r.data),
  reorderCategories: (tripId: number | string, orderedCategories: string[]) =>
    apiClient
      .put(`/trips/${tripId}/budget/reorder/categories`, { orderedCategories } satisfies BudgetReorderCategoriesRequest)
      .then((r) => r.data),
};

export const filesApi = {
  list: (tripId: number | string, trash?: boolean) =>
    apiClient.get(`/trips/${tripId}/files`, { params: trash ? { trash: 'true' } : {} }).then((r) => r.data),
  upload: (tripId: number | string, formData: FormData) =>
    apiClient
      .post(`/trips/${tripId}/files`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      .then((r) => r.data),
  update: (tripId: number | string, id: number, data: FileUpdateRequest) =>
    apiClient.put(`/trips/${tripId}/files/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number) => apiClient.delete(`/trips/${tripId}/files/${id}`).then((r) => r.data),
  toggleStar: (tripId: number | string, id: number) =>
    apiClient.patch(`/trips/${tripId}/files/${id}/star`).then((r) => r.data),
  restore: (tripId: number | string, id: number) =>
    apiClient.post(`/trips/${tripId}/files/${id}/restore`).then((r) => r.data),
  permanentDelete: (tripId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/files/${id}/permanent`).then((r) => r.data),
  emptyTrash: (tripId: number | string) => apiClient.delete(`/trips/${tripId}/files/trash/empty`).then((r) => r.data),
  addLink: (tripId: number | string, fileId: number, data: FileLinkRequest) =>
    apiClient.post(`/trips/${tripId}/files/${fileId}/link`, data).then((r) => r.data),
  removeLink: (tripId: number | string, fileId: number, linkId: number) =>
...
export const reservationsApi = {
  ...
  create: (tripId: number | string, data: ReservationCreateRequest) =>
    apiClient.post(`/trips/${tripId}/reservations`, data).then((r) => r.data),
  update: (tripId: number | string, id: number, data: ReservationUpdateRequest) =>
    apiClient.put(`/trips/${tripId}/reservations/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/reservations/${id}`).then((r) => r.data),
  updatePositions: (tripId: number | string, positions: { id: number; day_plan_position: number }[], dayId?: number) =>
    apiClient.put(`/trips/${tripId}/reservations/positions`, { positions, day_id: dayId }).then((r) => r.data),
  importBookingPreview: (tripId: number | string, files: File[]): Promise<BookingImportPreviewResponse> => {
    const fd = new FormData();
    for (const f of files) fd.append('files', f);
    return apiClient
      .post(`/trips/${tripId}/reservations/import/booking`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      .then((r) => r.data);
  },
  importBookingConfirm: (
    tripId: number | string,
...
export const weatherApi = {
  get: (lat: number, lng: number, date: string): Promise<WeatherResult> =>
    apiClient
      .get('/weather', { params: { lat, lng, date } })
      .then((r) => parseInDev(weatherResultSchema, r.data, 'weather.get')),
  getDetailed: (lat: number, lng: number, date: string, lang?: string): Promise<WeatherResult> =>
    apiClient
      .get('/weather/detailed', { params: { lat, lng, date, lang } })
      .then((r) => parseInDev(weatherResultSchema, r.data, 'weather.getDetailed')),
};

export const configApi = {
  getPublicConfig: (): Promise<{ defaultLanguage: string }> => apiClient.get('/config').then((r) => r.data),
};
...
export const accommodationsApi = {
  ...
  create: (tripId: number | string, data: AccommodationCreateRequest) =>
...
export const dayNotesApi = {
  list: (tripId: number | string, dayId: number | string) =>
    apiClient.get(`/trips/${tripId}/days/${dayId}/notes`).then((r) => r.data),
  create: (tripId: number | string, dayId: number | string, data: DayNoteCreateRequest) =>
    apiClient.post(`/trips/${tripId}/days/${dayId}/notes`, data).then((r) => r.data),
  update: (tripId: number | string, dayId: number | string, id: number, data: DayNoteUpdateRequest) =>
    apiClient.put(`/trips/${tripId}/days/${dayId}/notes/${id}`, data).then((r) => r.data),
  delete: (tripId: number | string, dayId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/days/${dayId}/notes/${id}`).then((r) => r.data),
};

export const collabApi = {
  getNotes: (tripId: number | string) => apiClient.get(`/trips/${tripId}/collab/notes`).then((r) => r.data),
  createNote: (tripId: number | string, data: CollabNoteCreateRequest) =>
    apiClient.post(`/trips/${tripId}/collab/notes`, data).then((r) => r.data),
  updateNote: (tripId: number | string, id: number, data: CollabNoteUpdateRequest) =>
    apiClient.put(`/trips/${tripId}/collab/notes/${id}`, data).then((r) => r.data),
  deleteNote: (tripId: number | string, id: number) =>
    apiClient.delete(`/trips/${tripId}/collab/notes/${id}`).then((r) => r.data),
  uploadNoteFile: (tripId: number | string, noteId: number, formData: FormData) =>
    apiClient
      .post(`/trips/${tripId}/collab/notes/${noteId}/files`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      .then((r) => r.data),
  ...
  votePoll: (tripId: number | string, id: number, optionIndex: number) =>
    apiClient
      .post(`/trips/${tripId}/collab/polls/${id}/vote`, { option_index: optionIndex } satisfies CollabPollVoteRequest)
      .then((r) => r.data),
  closePoll: (tripId: number | string, id: number) =>
    apiClient.put(`/trips/${tripId}/collab/polls/${id}/close`).then((r) => r.data),
  deletePoll: (tripId: number | string, id: number) =>
  ...
  reactMessage: (tripId: number | string, id: number, emoji: string) =>
    apiClient
      .post(`/trips/${tripId}/collab/messages/${id}/react`, { emoji } satisfies CollabReactionRequest)
      .then((r) => r.data),
...
export const backupApi = {
  ...
  uploadRestore: (file: File) => {
    ...
    return apiClient
      .post('/backup/upload-restore', form, { headers: { 'Content-Type': 'multipart/form-data' } })
      .then((r) => r.data);
  },
  getAutoSettings: () => apiClient.get('/backup/auto-settings').then((r) => r.data),
  setAutoSettings: (settings: Record<string, unknown>) =>
    apiClient.put('/backup/auto-settings', settings).then((r) => r.data),
};

export const shareApi = {
  getLink: (tripId: number | string) => apiClient.get(`/trips/${tripId}/share-link`).then((r) => r.data),
  createLink: (tripId: number | string, perms?: Record<string, boolean>) =>
...
export const inAppNotificationsApi = {
  list: (params?: { limit?: number; offset?: number; unread_only?: boolean }): Promise<InAppListResult> =>
    apiClient
      ...
      .then((r) => parseInDev(inAppListResultSchema, r.data, 'notifications.list')),
  unreadCount: (): Promise<UnreadCountResult> =>
    apiClient
      .get('/notifications/in-app/unread-count')
      .then((r) => parseInDev(unreadCountResultSchema, r.data, 'notifications.unreadCount')),
  markRead: (id: number) => apiClient.put(`/notifications/in-app/${id}/read`).then((r) => r.data),
  markUnread: (id: number) => apiClient.put(`/notifications/in-app/${id}/unread`).then((r) => r.data),
  markAllRead: () => apiClient.put('/notifications/in-app/read-all').then((r) => r.data),
  delete: (id: number) => apiClient.delete(`/notifications/in-app/${id}`).then((r) => r.data),
  deleteAll: () => apiClient.delete('/notifications/in-app/all').then((r) => r.data),
