...
export const createTodoSlice = (set: SetState, get: GetState): TodoSlice => ({
  ...
  toggleTodoItem: async (tripId, id, checked) => {
    set((state) => ({
      todoItems: state.todoItems.map((item) => (item.id === id ? { ...item, checked: checked ? 1 : 0 } : item)),
    }));
    try {
      ...
      set((state) => ({
        todoItems: state.todoItems.map((item) => (item.id === id ? { ...item, checked: checked ? 0 : 1 } : item)),
      }));
