...
export class ReservationMapboxOverlay {
  ...
  update(
    reservations: Reservation[],
    opts: ReservationOverlayOptions,
    transportRoutes: Record<number, MapsTransportRouteResult | undefined> = {}
  ) {
    this.opts = opts;
    this.items = buildItems(reservations, transportRoutes);
    this.render();
  }
  ...
  private updateStatsRotation = () => {
    ...
    for (const entry of this.statsMarkers) {
      ...
      try {
        ...
        const inner = el.querySelector('.trippi-stats-inner') as HTMLElement | null;
        if (inner) inner.style.transform = `rotate(${angle}deg)`;
      } catch {
        /* map not ready / projection failure */
      }
    }
  };
}
