...
import { getAccommodationAnchors, getDayBookendHotels, getDayOrder, isDayInAccommodationRange } from './dayOrder';
...
describe('getDayBookendHotels', () => {
  ...
  it('flags an arrival/check-in day as not slept-here in the morning (#1321)', () => {
    ...
    expect(getAccommodationAnchors(days[0], days, [into])).toEqual({
      start: { lat: 3, lng: 4 },
      end: { lat: 3, lng: 4 },
    });
  });
