...
function useCollabNotes({ tripId, currentUser }: CollabNotesProps) {
  ...
  const [emptyCategories, setEmptyCategories] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(`collab-cats-${tripId}`)) || {};
    } catch {
      return {};
    }
  });
  ...
  useEffect(() => {
    ...
    const handler = (msg) => {
      ...
      if (msg.type === 'collab:note:updated' && msg.note) {
        setNotes((prev) => prev.map((n) => (n.id === msg.note.id ? { ...n, ...msg.note } : n)));
      }
      ...
      if (created) {
        ...
        if (pendingFiles.length > 0 && note.id) {
          for (const file of pendingFiles) {
            ...
            fd.append('file', file);
            try {
              await collabApi.uploadNoteFile(tripId, note.id, fd);
            } catch (err) {
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
      {sortedNotes.length === 0 ? (
        ...
        >
          ...
          <div
            style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4, fontFamily: FONT }}
          >
