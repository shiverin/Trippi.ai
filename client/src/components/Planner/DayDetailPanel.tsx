...
function cTemp(c, f) {
  return Math.round(f ? (c * 9) / 5 + 32 : c);
}
...
export default function DayDetailPanel({
  ...
  const fmtTime = (v) => {
    if (!v) return v;
    if (v.includes('T'))
      return new Date(v).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit', hour12: is12h });
  ...
  const formattedDate = day.date
    ? new Date(day.date + 'T00:00:00Z').toLocaleDateString(getLocaleForLanguage(language), {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
        timeZone: 'UTC',
  ...
  return (
    <div
      ...
      style={{
        ...
        left: `calc(${leftWidth}px + (100vw - ${leftWidth}px - ${rightWidth}px) / 2)`,
        transform: 'translateX(-50%)',
        width: `min(800px, calc(100vw - ${leftWidth}px - ${rightWidth}px - 32px))`,
        ...(mobile ? { zIndex: 10000 } : null),
        ...font,
      }}
    >
      <div
      ...
      >
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            gap: 14,
            padding: collapsed ? '12px 16px 12px 20px' : '18px 16px 14px 20px',
            borderBottom: collapsed ? 'none' : '1px solid var(--border-faint)',
            cursor: 'pointer',
          }}
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div
              ...
              style={{ fontSize: collapsed ? 13 : 15, fontWeight: 700, transition: 'font-size 0.15s ease' }}
            >
              {day.title || t('planner.dayN', { n: days.indexOf(day) + 1 || '?' })}
              {collapsed && formattedDate && (
                <span className="text-content-muted" style={{ fontWeight: 500, marginLeft: 8 }}>
                  {formattedDate}
                </span>
              )}
            </div>
            {!collapsed && formattedDate && (
              <div className="text-content-muted" style={{ fontSize: 12, marginTop: 1 }}>
                {formattedDate}
              </div>
            )}
          </div>
          ...
          >
            {collapsed ? (
              <ChevronsUp size={14} className="text-content-muted" />
            ) : (
              <ChevronsDown size={14} className="text-content-muted" />
            )}
          </button>
          <button
            onClick={(e) => {
        ...
        <div style={{ overflowY: 'auto', padding: '14px 20px 18px', display: collapsed ? 'none' : 'block' }}>
            ...
            (loading ? (
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1 }}>
                      ...
                      {cTemp(weather.temp, isFahrenheit)}
                      {unit}
                    </span>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: weather.hourly ? 10 : 0 }}>
                  ...
                  {weather.wind_max != null && (
                    <Chip
                      ...
                      value={
                        isFahrenheit
                          ? `${Math.round(weather.wind_max * 0.621371)} mph`
                    ...
                    <div style={{ display: 'inline-flex', gap: 2 }}>
                      {weather.hourly
                        ...
                        .map((h) => (
                          ...
                          >
                            <span style={{ fontSize: 9, color: 'var(--text-faint)', fontWeight: 500 }}>
                            ...
                            {h.precipitation_probability > 0 && (
                              <span style={{ fontSize: 8, color: '#3b82f6', fontWeight: 500 }}>
                ...
                {weather.type === 'climate' && (
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 6, fontStyle: 'italic' }}>
                    {t('day.climateHint')}
                  </div>
                )}
              </div>
            ) : (
              <div style={{ fontSize: 12, color: 'var(--text-faint)', textAlign: 'center', padding: 8 }}>
                {t('day.noWeather')}
              </div>
            ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {dayReservations.map((r) => {
                    ...
                    return (
                      ...
                      >
                        {(() => {
                          const TIcon = RES_TYPE_ICONS[r.type] || FileText;
                          return (
                            <TIcon
                              size={12}
                              style={{ color: RES_TYPE_COLORS[r.type] || 'var(--text-faint)', flexShrink: 0 }}
                            />
                          );
                        })()}
                        <div
                        ...
                        {(() => {
                          ...
                          return (
                            <span
                              style={{ fontSize: 10, color: 'var(--text-muted)', whiteSpace: 'nowrap', flexShrink: 0 }}
                            >
          ...
          <div>
            ...
            <AccommodationList
              ...
              setHotelDayRange={setHotelDayRange}
              setShowHotelPicker={setShowHotelPicker}
              handleRemoveAccommodation={handleRemoveAccommodation}
            />

            <HotelPickerModal
              showHotelPicker={showHotelPicker}
              setShowHotelPicker={setShowHotelPicker}
              font={font}
              t={t}
              ...
              setHotelCategoryFilter={setHotelCategoryFilter}
              places={places}
              ...
              setDayAccommodations={setDayAccommodations}
              setAccommodation={setAccommodation}
                  ...
                  <div style={{ padding: '4px 12px 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <span
                      style={{
                        ...
                        textTransform: 'uppercase',
                        letterSpacing: '0.05em',
                        color: isCheckOutDay && !isCheckInDay ? '#ef4444' : '#22c55e',
                      }}
                    >
                      {dayLabel}
                    </span>
                  ...
                  {acc.check_in && (
                    ...
                    >
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.2 }}>
                        ...
                        {acc.check_in_end ? ` – ${fmtTime(acc.check_in_end)}` : ''}
                      </div>
                      <div
                        style={{
                          fontSize: 9,
                  ...
                  {acc.check_out && (
                    <div
                      style={{
                        flex: 1,
                        padding: '8px 10px',
                        borderRight: acc.confirmation ? '1px solid var(--border-faint)' : 'none',
                        textAlign: 'center',
                      }}
                    >
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.2 }}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 6,
                        borderRadius: '50%',
                        background: confirmed ? '#16a34a' : '#d97706',
                        flexShrink: 0,
                      }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 9, color: 'var(--text-faint)', display: 'flex', gap: 6, marginTop: 1 }}>
                        ...
                        {linked.confirmation_number && (
                          <span
                            onMouseEnter={(e) => {
                              if (blurCodes) e.currentTarget.style.filter = 'none';
                            }}
                            onMouseLeave={(e) => {
                              if (blurCodes) e.currentTarget.style.filter = 'blur(4px)';
                            }}
                            onClick={(e) => {
                              if (blurCodes) {
                            ...
                            style={{
                              filter: blurCodes ? 'blur(4px)' : 'none',
          ...
          >
            ...
            >
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <CustomSelect
                      ...
                      onChange={(v) =>
                        setHotelDayRange((prev) => ({
                          start: v,
                            ...
                            days.findIndex((d) => d.id === v) > days.findIndex((d) => d.id === prev.end) ? v : prev.end,
                        }))
                      }
                      options={days.map((d, i) => ({
                        ...
                        badge: d.date
                          ? new Date(d.date + 'T00:00:00Z').toLocaleDateString(locale, {
                              day: 'numeric',
                              month: 'short',
                              timeZone: 'UTC',
                            })
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <CustomSelect
                      ...
                      onChange={(v) =>
                        setHotelDayRange((prev) => ({
                          start:
                            days.findIndex((d) => d.id === v) < days.findIndex((d) => d.id === prev.start)
                              ? v
                              : prev.start,
                          end: v,
                        }))
                  ...
                  <button
                    ...
                    style={{
                      ...
                      flexShrink: 0,
                      background:
                        hotelDayRange.start === days[0]?.id && hotelDayRange.end === days[days.length - 1]?.id
                          ? 'var(--text-primary)'
                          : 'var(--bg-card)',
                      color:
                        hotelDayRange.start === days[0]?.id && hotelDayRange.end === days[days.length - 1]?.id
              ...
              >
                <div style={{ flex: 1, minWidth: 80 }}>
                  ...
                  <CustomTimePicker
                    ...
                    onChange={(v) => setHotelForm((f) => ({ ...f, check_in: v }))}
                    placeholder="14:00"
                  />
                </div>
                <div style={{ flex: 1, minWidth: 80 }}>
                  ...
                  <CustomTimePicker
                    value={hotelForm.check_in_end}
                    onChange={(v) => setHotelForm((f) => ({ ...f, check_in_end: v }))}
                    placeholder="22:00"
                  />
                </div>
                <div style={{ flex: 1, minWidth: 80 }}>
                  ...
                  <CustomTimePicker
                    ...
                    onChange={(v) => setHotelForm((f) => ({ ...f, check_out: v }))}
                    placeholder="11:00"
                  />
                </div>
                <div style={{ flex: 2, minWidth: 120 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setHotelForm((f) => ({ ...f, confirmation: e.target.value }))}
              ...
              {categories.length > 0 && (
                ...
                >
                  <button
                    onClick={() => setHotelCategoryFilter('')}
                    style={{
                      padding: '3px 10px',
                      borderRadius: 6,
                      border: 'none',
                  ...
                  {categories.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => setHotelCategoryFilter(c.id)}
                      style={{
                        padding: '3px 10px',
                        borderRadius: 6,
                        border: 'none',
                        ...
                        cursor: 'pointer',
                        background:
                          hotelCategoryFilter === c.id ? c.color || 'var(--text-primary)' : 'var(--bg-secondary)',
              ...
              <div style={{ maxHeight: 250, overflowY: 'auto' }}>
                {(() => {
                  const filtered = hotelCategoryFilter
                    ? places.filter((p) => p.category_id === hotelCategoryFilter)
                    : places;
                    ...
                    filtered.map((p) => (
                      <button
                        ...
                        style={{
                          ...
                          outlineOffset: -2,
                          borderRadius: hotelForm.place_id === p.id ? 8 : 0,
                        }}
                        onMouseEnter={(e) => {
                          if (hotelForm.place_id !== p.id) e.currentTarget.style.background = 'var(--bg-hover)';
                        }}
                        onMouseLeave={(e) => {
                          if (hotelForm.place_id !== p.id) e.currentTarget.style.background = 'none';
                        }}
                      >
                        <div
                          style={{
