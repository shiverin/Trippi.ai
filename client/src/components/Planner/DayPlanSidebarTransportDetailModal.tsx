        ...
        {(() => {
          ...
          if (res.type === 'flight') {
            ...
            if (meta.flight_number)
              detailFields.push({ label: t('reservations.meta.flightNumber'), value: meta.flight_number });
            if (meta.departure_airport)
              detailFields.push({ label: t('reservations.meta.from'), value: meta.departure_airport });
            if (meta.arrival_airport)
              detailFields.push({ label: t('reservations.meta.to'), value: meta.arrival_airport });
            if (meta.seat) detailFields.push({ label: t('reservations.meta.seat'), value: meta.seat });
          } else if (res.type === 'train') {
            if (meta.train_number)
              detailFields.push({ label: t('reservations.meta.trainNumber'), value: meta.train_number });
          ...
          return (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div className="text-content" style={{ fontSize: 15, fontWeight: 600 }}>
                    {res.title}
                  </div>
              ...
              {/* Dateien */}
              {(() => {
                const resFiles = (useTripStore.getState().files || []).filter(
                  (f) =>
                    !f.deleted_at &&
